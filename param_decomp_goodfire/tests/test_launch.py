...
_MINIMAL_LM = {
    ...
    "runtime": {"device": "cuda:0", "dp": 1, "sharding": "zero1"},
...
def test_validate_config_fires_the_placement_claim_gate_pre_submit(tmp_path: Path):
    ...
    (here the single-site smoke at `dp: 1`) refuses on the login node."""
    raw = dict(_MINIMAL_LM, runtime={"dp": 1, "sharding": "owner+zero1"})
    config = tmp_path / "c.yaml"
