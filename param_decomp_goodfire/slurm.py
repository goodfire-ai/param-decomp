...
from param_decomp_goodfire.git import snapshot_source_repo
...
class SlurmConfig:
    ...
    just `cd` to the repo root. `dependency_job_id` adds an `afterok` dependency.
    """
...
def submit_slurm_job(
    ...
    Writes the script to `GENV.sbatch_scripts_dir`, submits via `sbatch`, renames the script
    to include the job ID, and `touch`es empty log file(s) for tailing.
    `n_array_tasks=None` is a singleton job.
    """
    GENV.sbatch_scripts_dir.mkdir(parents=True, exist_ok=True)
    GENV.slurm_logs_dir.mkdir(parents=True, exist_ok=True)

    # Write script to a unique temporary file (safe for concurrent submissions)
    with tempfile.NamedTemporaryFile(
        mode="w",
        dir=GENV.sbatch_scripts_dir,
    ...
    final_script_path = GENV.sbatch_scripts_dir / f"{script_name_prefix}_{job_id}.sh"
    ...
    if n_array_tasks is not None:
        for i in range(1, n_array_tasks + 1):
            (GENV.slurm_logs_dir / f"slurm-{job_id}_{i}.out").touch()
        log_pattern = str(GENV.slurm_logs_dir / f"slurm-{job_id}_*.out")
    else:
        (GENV.slurm_logs_dir / f"slurm-{job_id}.out").touch()
        log_pattern = str(GENV.slurm_logs_dir / f"slurm-{job_id}.out")
...
def _common_sbatch_lines(config: SlurmConfig, log_pattern: str) -> list[str]:
    ...
    lines = [
        ...
        f"#SBATCH --output={GENV.slurm_logs_dir}/slurm-{log_pattern}.out",
