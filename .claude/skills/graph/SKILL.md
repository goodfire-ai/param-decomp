...
- **Sign interpretation**: Edge `val` is an attribution, not a direct contribution. The actual effect on the target depends on the product of the source node's activation and the attribution. A negative `val` from a negatively-activated component is a **positive** contribution. Component activations are available in `graph_data['nodeSubcompActs']`.
