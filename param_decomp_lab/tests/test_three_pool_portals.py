...
def _run() -> None:
    ...
    try:
        ...
        if isinstance(ctx, PPGDContext):
            ...
            want = {
                s: _pattern(_slice_rows(ctx.role.slice_idx, world.batch_local_ppgd), s)
                for s in _SITES
            }
            for s in _SITES:
        ...
        if isinstance(ctx, ChunkContext):
            got = ctx.portals.ci_from_ci_pool.post_recv(ctx.role, _C, _SEQ, cpu).wait()
            want = {
                s: _pattern(_slice_rows(ctx.role.within_chunk_idx, world.batch_local_chunk), s)
                for s in _SITES
            }
            for s in _SITES:
                torch.testing.assert_close(
        ...
        if isinstance(ctx, CIContext):
            ...
            want = {
                s: _pattern(_slice_rows(ctx.role.slice_idx, world.batch_local_ci), s)
                for s in _SITES
            }
            for s in _SITES:
                torch.testing.assert_close(
