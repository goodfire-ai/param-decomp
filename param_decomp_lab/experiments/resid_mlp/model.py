...
def _run_masked(
    ...
    """The masked decomposed forward: each layer's `mlp_in`/`mlp_out` runs
...
def masked_site_outputs(
    ...
    """Per-`live`-site decomposed output of the masked forward."""
...
def weight_deltas_fp32(
    ...
    """fp32 `W − (V@U)ᵀ` per site from fp32 masters (faithfulness input)."""
    out: dict[str, Array] = {}
    for spec in sites:
