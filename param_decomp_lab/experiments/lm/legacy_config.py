...
# Pretrain models migrated through two namespace renames: first `spd.pretrain.*` →
# `param_decomp.pretrain.*`, then both → `param_decomp_lab.experiments.lm.pretrain.*`
# in the package split. Legacy configs may reference either of the older paths.
_PRETRAIN_MODULE_RENAMES = {
    "spd.pretrain.": "param_decomp_lab.experiments.lm.pretrain.",
    "param_decomp.pretrain.": "param_decomp_lab.experiments.lm.pretrain.",
}
...
def _migrate_optimizer_block(block: dict[str, Any]) -> dict[str, Any]:
    """Strip the legacy `type: AdamW` discriminator value.

    Legacy configs only ever used AdamW, tagged `type: AdamW`. The current optimizer
    union discriminates on a lowercase `type` (`adamw` / `muon`) and defaults to `adamw`
    when absent, so dropping the legacy capitalized value lands these on AdamW.
    """
...
def translate_legacy_lm_config(legacy_path: Path) -> dict[str, Any]:
    ...
    - Oldest: single top-level `lr_schedule` + `grad_clip_norm_components` / `grad_clip_norm_ci_fns`.
    - Slightly newer: nested `components_optimizer` / `ci_fn_optimizer` blocks already shaped like
      the current `AnyOptimizerConfig` (modulo a `type: AdamW` discriminator that we strip).
    ...
    if isinstance(old.get("components_optimizer"), dict):
        components_optimizer = _migrate_optimizer_block(old["components_optimizer"])
        ci_fn_optimizer = _migrate_optimizer_block(old["ci_fn_optimizer"])
    else:
        components_optimizer = {"lr_schedule": old["lr_schedule"]}
        if old.get("grad_clip_norm_components") is not None:
            components_optimizer["grad_clip_norm"] = old["grad_clip_norm_components"]
        ci_fn_optimizer = {"lr_schedule": old["lr_schedule"]}
        if old.get("grad_clip_norm_ci_fns") is not None:
            ci_fn_optimizer["grad_clip_norm"] = old["grad_clip_norm_ci_fns"]

    legacy_alive_threshold = old.get("ci_alive_threshold")
    assert legacy_alive_threshold in (None, 0, 0.0), (
        f"Legacy `ci_alive_threshold={legacy_alive_threshold}` cannot be translated; "
        f"new schema has no equivalent (config at {legacy_path})"
    )
    ...
    # The newer legacy variant stored `output_extract` directly at top level; the older
    # one used `pretrained_model_output_attr` (with `idx_N` strings translated to ints).
    if "output_extract" in old:
        output_extract = old["output_extract"]
    else:
