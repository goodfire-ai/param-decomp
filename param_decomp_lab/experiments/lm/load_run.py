...
def build_target(cfg: BuiltRun, mesh: jax.sharding.Mesh) -> tuple[DecomposedModel, int]:
    ...
    match cfg.target:
        ...
        case TargetConfig():
            arch_cfg = hf_model_config(cfg.target.model_name)
            sites = llama_site_specs(arch_cfg, cfg.target.sites)
            lm = place_target(
                load_decomposed_lm_from_hf(
...
def run_metadata(run_dir: Path) -> RunMetadata:
    ...
    match cfg.target:
        ...
        case TargetConfig():
            arch_cfg = hf_model_config(cfg.target.model_name)
            return RunMetadata(
                model_type="Qwen3" if "Qwen3" in cfg.target.model_name else "Llama",
