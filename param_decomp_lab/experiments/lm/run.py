...
def _maybe_make_torch_profiler(
    rank: int,
    pool: Literal["ci", "layerwise", "ppgd"],
) -> PhaseProfiler | None:
    """Opt-in torch.profiler bound to the rank's PhaseProfiler instance.

    Activated by env vars:
      * ``PD_TORCH_PROFILE_RANKS=0,96,104`` — comma-separated ranks to profile.
      * ``PD_TORCH_PROFILE_OUT=/abs/path/to/dir`` — dump directory.

    Optional knobs:
      * ``PD_TORCH_PROFILE_SKIP_FIRST`` (default 5) — warmup iters before recording.
      * ``PD_TORCH_PROFILE_ACTIVE`` (default 3) — iters to record into the trace.

    Dumps ``trace_<pool>_rank<R>.json`` (Chrome trace) + ``key_avgs_<pool>_rank<R>.txt``
    (top ops by self-CUDA time). Load the json into https://www.speedscope.app/.
    """
    prof_ranks_env = os.environ.get("PD_TORCH_PROFILE_RANKS")
    if not prof_ranks_env:
        return None
    if rank not in {int(r) for r in prof_ranks_env.split(",") if r.strip()}:
        return None
    ...
    out_dir.mkdir(parents=True, exist_ok=True)
    skip_first = int(os.environ.get("PD_TORCH_PROFILE_SKIP_FIRST", "5"))
    active = int(os.environ.get("PD_TORCH_PROFILE_ACTIVE", "3"))
    logger.info(
        f"[torch-profile] recording rank={rank} pool={pool} "
        f"skip_first={skip_first} active={active} → {out_dir}"
    )
...
def _construct_and_run_trainer(
    *,
    ...
    elif isinstance(trainer, ThreePoolTrainer):
        profiler = _maybe_make_torch_profiler(
            rank=trainer.layout.my_rank, pool=trainer.layout.my_pool
        )
        trainer.run(train_loader, sink, cfg.cadence, profiler=profiler)
    else:
