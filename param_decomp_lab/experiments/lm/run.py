...
def _submit_slurm(
    ...
    *,
    ...
    base_parts = ["-m", "param_decomp_lab.experiments.lm.run", config_arg]
    if group is not None:
        base_parts.extend(["--group", group])
    if tags is not None:
        base_parts.extend(["--tags", tags])
    base_parts.extend(["--run_id", run_id])
    base_command = " ".join(shlex.quote(part) for part in base_parts)
