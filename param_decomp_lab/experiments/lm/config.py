...
GLU_FAMILY = ArchFamily("glu_transformer", llama8b.KIND_ORDER, llama8b.site_name)
...
def _resolve_decomposition(cfg: LMExperimentConfig) -> _ResolvedDecomposition:
    ...
    Vendored and raw-HF Llama specs load the SAME meta-llama weights (GLU family); `kind:
    ...
    match spec:
        case HFWeightsInVendored() | HFTarget():
            ...
            tree = resolve_site_tree(sites, GLU_FAMILY, llama8b.llama31_8b_config().n_layer)
            target = TargetConfig(model_name=spec.model_name, sites=tree.site_cs(GLU_FAMILY.name_of))
...
def _resolved_chunks(
    ...
    for start in range(0, len(blocks), blocks_per_chunk):
        ...
        chunks.append(
            Chunk(
                input_taps=_chunk_input_taps(input_tap, [block.layer_idx for block in group]),
...
def _resolve_chunkwise_ci_arch(
    ...
    the per-chunk input width (one residual tap of width `d_resid`)."""
