...
def _migrate_legacy_data_config(config_dict: dict[str, Any]) -> dict[str, Any]:
    ...
    # `shuffle_each_epoch` post-dates the oldest runs (absent in both splits then). Require
    # equality only when present; default True. Irrelevant to decomposition, which supplies
    # its own data config.
