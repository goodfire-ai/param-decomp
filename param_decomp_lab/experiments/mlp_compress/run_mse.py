...
Run: python -m param_decomp_lab.experiments.mlp_compress.run_mse --n_heads H --n_compressed N [--steps ...] ...
"""
...
from param_decomp_lab.experiments.mlp_compress.run_attn import (
    ORIG_HEAD_DIM,
    ORIG_N_HEADS,
...
def main(
    ...
    if use_wandb:
        wb = wandb.init(
            ...
            group="attn_mlp_relmse_sparsity_relu_bypass_sweep"
            if bypass
            else "attn_mlp_relmse_sparsity_relu_sweep",
    ...
    with torch.no_grad(), autocast():
        for b in eval_batches:
            with capture_residuals(comp_model, n_blocks) as tr:
                ci, mask_infos, target_logits, teacher_logits = compute_ci_and_teacher(
                    comp_model, b
                )
    ...
    for step in range(steps):
        ...
        relmse_total = torch.stack(list(relmse_terms.values())).sum()
        sparsity_terms = {
            blk: lp_sparsity_penalty(compressed_mlp[blk].last_hidden.float(), p_norm)
            for blk in range(n_blocks)
        }
        sparsity_total = torch.stack(list(sparsity_terms.values())).sum()
