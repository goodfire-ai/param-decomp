...
Run: python -m param_decomp_lab.experiments.mlp_compress.run [--steps N] [--batch_size B] ...
"""
...
class CompressedMaskedMLP(nn.Module):
    ...
    def __init__(
        ...
        self.last_hidden = torch.empty(0)  # neuron pre-acts of the last forward (read for sparsity)
...
) -> tuple[dict[str, Tensor], dict[str, ComponentsMaskInfo], Tensor, Tensor]:
...
def main(
    ...
    with torch.no_grad(), autocast():
        ...
        for b, ci, _mask_infos, target_logits, teacher_logits in eval_ctx:
            ...
            zeroed_ci = dict(ci) | {
                CFC: torch.zeros_like(ci[CFC]),
                DOWN: torch.zeros_like(ci[DOWN]),
            }
