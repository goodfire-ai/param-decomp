...
def load_prompt_tokens(
    ...
    `add_special_tokens` must match the eval probe's tokenization: an arithmetic probe encodes
    with add_special_tokens=True (Llama BOS included), so the target prompts must too, else the
    target forward and the probe disagree on the leading BOS."""
