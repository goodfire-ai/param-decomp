"""GPN-MSA PD experiment: YAML -> bundle/runner glue, plus the `SavedGPNMSARun` reload class.
...
def _build_eval_loop(
    cfg: GPNMSAExperimentConfig, device: str, dist_state: DistributedState | None
) -> EvalLoop | None:
    if cfg.eval is None:
        return None
...
_GPN_MSA_BUNDLE = ExperimentBundle[GPNMSAExperimentConfig](
    config_cls=GPNMSAExperimentConfig,
    build_target=lambda cfg: build_target(cfg.target),
    build_train_loader=lambda cfg, device, dist_state: build_gpn_msa_loader(
        cfg.target,
        cfg.data,
        split="train",
        device=device,
...
def main(
    ...
    *,
    ...
    run_fresh(_GPN_MSA_BUNDLE, Path(config_path), group=group, tags=tags)
