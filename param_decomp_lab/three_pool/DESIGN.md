...
Splits training across three heterogeneous GPU pools — CI, chunkwise (V/U), and
PPGD — so a **global shared transformer** CI fn is physically realizable: a
dedicated, replicated CI pool can host a CI fn that spans all sites, while V/U
sites are sharded into chunks across the chunkwise pool.

("Chunkwise" — not "layerwise": a chunk owns an arbitrary slice of the decomposed
*sites*, not a model layer. The chunkwise pool *computes* a core-library layerwise
recon loss — `StochasticReconLayerwiseLoss`, which keeps that name — but the pool
itself shards sites into chunks.)
...
| **Chunkwise** | V/U + V/U optimizer state    | by site (chunks) + DP across batch within a chunk | Chunkwise stoch recon + faithfulness. Same shape as today's Pool A minus the CI fn. |
...
| `config.py` | `ThreePoolTopology` — per-rank batch per pool + `sites_per_chunk`; `resolve(ordered_sites, batch_size)` derives the canonical rank/chunk assignment (`ResolvedLayout`). No rank ids authored. |
| `role.py` | `PoolRole = CIRole \| ChunkRole \| PPGDRole` — this rank's view. Each variant carries ONLY the fields valid for its pool (no `\| None` bag). Replaces the old `ThreePoolLayout` optional-attr bag + `assert self.my_pool == ...` guards. |
| `portals.py` | One typed object per cross-pool DAG edge. Each defines its payload, routing, pack/unpack, process group, and reduction/placement ONCE; sender + receiver are two methods on the SAME class, so they can't drift. Also the three in-pool collectives. Per-pool bundles (`CIPortals` / `ChunkPortals` / `PPGDPortals`) expose only the edges that pool participates in. |
| `context.py` | `PoolContext = CIContext \| ChunkContext \| PPGDContext` — bundles `world` + `role` + that pool's portal bundle. The trainer builds one and `match`es it; each step fn receives its specific context type. |
| `step_{ci,chunkwise,ppgd}.py` | The per-pool step bodies. Take their specific `*Context`, drive their portals. Each body is a sequence of typed phase calls: every phase fn consumes the prior phase's frozen output bundle (CI: `CiForward`→`CiSends`/`ImpMinLoss`→`GciReceived`→`GciTotal`; chunkwise: `Faith`/`CiLeaves`→`Stoch`; PPGD: `ReconSum`→`RawGrads`), so an invalid phase order (e.g. send CI grads before the backward populated them, or differentiate before warmup refined the sources) is a type error, not just a runtime assert. |
...
| Edge | Portal | Payload |
|---|---|---|
| CI → chunk   | `CiValuesToChunkwise` | per-site CI values (chunk sites, chunk-rank batch sub-slice) |
| CI → PPGD    | `CiValuesToPPGD`      | full-model CI values (PPGD-rank batch sub-slice) |
| chunk → CI   | `GradCiFromChunkwise` | per-site CI grads (per-chunk-rank slice, stitched on CI side) |
| PPGD → CI    | `GradCiFromPPGD`      | full-model CI grads (per-PPGD-rank slice, stitched on CI side) |
| PPGD → chunk | `GradVuFromPPGD`      | per-site V/U grads (after PPGD in-pool sum-reduce) |
| chunk → PPGD | `UpdatedVuToPPGD`     | updated V/U (leader-rooted broadcast over {leader} ∪ PPGD) |

Eval adds `CiOutputsEvalToPPGD` (CI → PPGD, full `CIOutputs`).

### What became unrepresentable

* **Wrong-pool comm.** A chunkwise step holds only `ChunkPortals`; it has no handle
  to the CI pool's `GradCiFromPPGD.recv` or to `UpdatedVuToPPGD.post_recv`. The
...
* **Reading a field that doesn't exist for this pool.** `CIRole` has no
  `sites`; `ChunkRole` has no `slice_idx`. The old
...
flowchart TD
    ...
    subgraph CI["CI pool · multi-rank DP across batch"]
        ...
        A2_LW["A2a · send CI_T per-site → Chunkwise<br/>routed by owner + batch slice"]:::ci
        ...
        A5["A5 · recv g_CI_chunk from Chunkwise<br/>per-site, per-chunk-rank slice"]:::ci
        A6["A6 · recv g_CI_PPGD from PPGD<br/>full-model, per-PPGD-rank slice"]:::ci
        A7["A7 · assemble g_CI_total per CI rank's batch slice<br/>= g_CI_imp + slice of g_CI_chunk + slice of g_CI_PPGD"]:::ci
    ...
    subgraph CH["Chunkwise pool · sharded by site · DP within chunk"]
        direction TB
        B0["V/U updated in T-1 dead time"]:::lw
        B1["B1 · target_fwd batch T → L_T<br/>per chunk rank's batch slice"]:::lw
        B2["B2 · wait for CI_T owned sites/slice"]:::lw
        B3["B3 · layerwise stoch recon, per owned site, streaming<br/>→ g_VU_chunk owned, g_CI_chunk owned/slice"]:::lw
        B4["B4 · faithfulness loss sharded across owned sites<br/>→ g_VU_faith owned"]:::lw
        B5["B5 · send g_CI_chunk → CI pool"]:::lw
        B6["B6 · recv g_VU_PPGD owned ← PPGD"]:::lw
        B7["B7 · combine V/U grads: g_VU_chunk + g_VU_faith + g_VU_PPGD"]:::lw
        B8["B8 · in-chunk all-reduce on V/U grads + faithfulness grads"]:::lw
    ...
    subgraph PG["PPGD pool · DP across batch · replicated V/U"]
        ...
        C7["C7 · send g_VU_PPGD owned → owning chunk rank"]:::pgd
        C8["C8 · send g_CI_PPGD slice → CI pool"]:::pgd
        C9["C9 · recv updated V/U ← Chunkwise<br/>(blocking, end of step)"]:::pgd
...
sequenceDiagram
    ...
    participant CH as Chunkwise pool
    participant PG as PPGD pool

    Note over CI: A1 · CI fn fwd on H_T → CI_T
    Note over CH: B1 · target_fwd batch T → L_T (chunk rank's slice)
    Note over PG: C1 · target_fwd batch T → L_T (PPGD rank's slice)

    CI->>CH: A2a · CI_T per-site, owned + chunk-rank slice
    ...
    and Chunkwise recon
        Note over CH: B3 · layerwise stoch recon (per owned site, streaming)<br/>→ g_VU_chunk owned, g_CI_chunk owned/slice
        Note over CH: B4 · faithfulness (sharded over owned sites)<br/>→ g_VU_faith owned
    and PPGD recon
    ...
    CH->>CI: B5 · g_CI_chunk (per-site, per-chunk-rank slice)
    PG->>CI: C8 · g_CI_PPGD (full-model, per-PPGD-rank slice)
    PG->>CH: C7 · g_VU_PPGD (per-owned-site, to owning chunk rank)
    ...
    Note over CH: B7 · combine V/U grads (chunkwise + faith + PPGD)
    Note over CH: B8-B9 · in-chunk all-reduce on V/U grads + AdamW step
    CH->>PG: B10/C9 · ship updated V/U → PPGD (blocking recv on PPGD)
    ...
    Note over CH: T+1.B1 · target_fwd batch T+1
    Note over PG: T+1.C1 · target_fwd batch T+1
    CI->>CH: T+1.A2a · CI_T+1 per-site
...
- Chunkwise rank `j` needs CI values for its owned sites `O[j]` and batch slice `S_chunk[j]`.
- PPGD rank `k` needs CI values for all sites and batch slice `S_pgd[k]`.

In general `S_ci[i]`, `S_chunk[j]`, `S_pgd[k]` don't align — so the CI→chunk and
...
batch splits so each chunk slice and each PPGD slice fits inside exactly one CI
...
1. **Batch-split divisibility.** Pick an N_ci / N_lw / N_pgd compatibility
   rule. Easiest: `N_ci` divides `N_pgd` and `chunk_dp`. Validator should
   reject anything else loudly.
2. **CI value wire dtype.** Today CI is shipped bf16 and upcast to fp32 on the
   chunkwise side. Same pattern likely works for both downstream pools.
3. **Where do imp_min grads enter the CI-fn backward?** Cleanest is: imp_min
   ...
   single CI-fn backward, with `g_CI_chunk + g_CI_PPGD` injected as a `grad_tensors=`
...
### 1. **chunkwise: post async recv_g_vu early, wait at combine**
...
state through D5 in step_chunkwise. Modest (~50 LOC).
...
Currently `recv_g_ci_from_chunkwise` and `recv_g_ci_from_ppgd` are two
...
recv is dominated by waiting for chunkwise/PPGD to send their grads; this
refactor doesn't change that, just removes the per-call NCCL setup overhead.
Small but real (~5-10 ms).

### 3. **CI: async in-pool all_reduce on CI fn grads (mirror the chunkwise pattern)** — DONE
...
### 5. **chunkwise: send_g_ci + recv_g_vu overlap**
...
before zero_grad + faith_bwd on chunkwise (we need fresh V/U... wait no, faith
