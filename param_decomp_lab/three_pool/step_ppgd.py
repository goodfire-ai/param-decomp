...
def step_ppgd(
    ...
    *,
    ...
    with strategy.context(component_model.target_model):
        ...
        ci_scratch = _wait_ci_and_releaf(ci_recv_pending, layout, seq_len, cfg)
    ...
    _scale_grads(raw, recon.n_examples, layout, cfg)
    ...
    portals.grad_ci_from_ppgd.send(raw.ci, my_ppgd_slice_idx=layout.my_ppgd_slice_idx)
    ...
    layout.sum_reduce_ppgd_grads([*raw.v.values(), *raw.u.values()])
    portals.grad_vu_from_ppgd.send(raw.v, raw.u, is_pool_leader=layout.my_is_pool_leader)
    ...
    v_new, u_new = portals.updated_vu_to_ppgd.recv(v_templates, u_templates)
...
def _post_ci_recv(
    ...
    assert layout.my_ppgd_slice_idx is not None
    return portals.ci_values_to_ppgd.post_recv(
...
def _scale_grads(
    raw: RawGrads, n_examples_local: int, layout: ThreePoolLayout, cfg: _ThreePoolRuntime
) -> None:
...
def _autograd_grads_wrt_vu_ci_and_sources(
    ...
    caller applies each consumer's own normalization afterward (see ``_scale_grads``),
