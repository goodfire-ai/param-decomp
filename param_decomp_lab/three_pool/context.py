...
``PoolContext = CIContext | LWContext | PPGDContext``. The trainer builds one
...
has no handle to ``UpdatedVuToPPGD``, an LW step has no handle to the CI-pool's
``GradCiFromPPGD``, and so on.
"""
...
from param_decomp_lab.three_pool.role import CIRole, LWRole, PPGDRole, role_for_rank
...
PoolContext = CIContext | LWContext | PPGDContext
...
def build_pool_context(world: World, rank: int) -> PoolContext:
    ...
    match role:
        ...
        case LWRole():
            return LWContext(world=world, role=role, portals=build_lw_portals(world, role))
        case PPGDRole():
