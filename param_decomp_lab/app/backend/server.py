...
Usage:
    python -m param_decomp_lab.app.backend.server --port 8000
"""
...
from param_decomp_lab.app.backend.routers.prompts import router as prompts_router
...
async def lifespan(app: FastAPI):  # pyright: ignore[reportUnusedParameter]
    ...
    from param_decomp_lab.app.backend.routers.mcp import (
        InvestigationConfig,
    ...
    if PARAM_DECOMP_APP_DEFAULT_RUN is not None:
        from param_decomp_lab.app.backend.routers.runs import load_run
...
def cli(port: int = 8000) -> None:
    ...
    uvicorn.run(
        "param_decomp_lab.app.backend.server:app",
        host="0.0.0.0",
        port=port,
        log_level="info",
    )
