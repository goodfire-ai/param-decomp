...
class SourceGradAlignment(Metric[SourceGradAlignmentConfig]):
    ...
    def _source_grads(
        ...
        return {
            k: all_reduce(g, op=ReduceOp.AVG) for k, g in zip(sources.keys(), grads, strict=True)
        }
    ...
    def update(self, ctx: MetricContext) -> None:
        ...
        for _ in range(self.cfg.n_pairs):
            ...
            for module_name in self.model.target_module_paths:
                mask_c = self.model.module_to_c[module_name] + (1 if ctx.use_delta_component else 0)
