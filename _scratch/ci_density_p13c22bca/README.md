...
| `harvest_ci_hist.py` | yes | Restores the run, accumulates an **unreduced per-token CI histogram** per component (no mean), streamed over the eval batches. Two binnings from the *same* forward: **log** (90 bins over `[1e-9, 1]` + bin-0 exact-zero underflow → `ci_hist_*.npz`) and **linear** (40 uniform 0.025 bins over `[0, 1]` + a per-component exact-0 counter `zero__<site>` → `ci_hist_lin_*.npz`). Saves per-component `(C, N_BINS)` counts + CI sums at 131k / 2.1M / 33.6M tokens. Never materialises the full `(tokens × C)` tensor. |
| `plot_ci_hist.py` | no | The per-token CI **density heatmap** from the cached log-binned `ci_hist_*.npz`. x = component (sorted desc by mean CI @ 33.6M, binned to 256 columns), y = per-token CI log `[1e-6, 1]`, cell = column-normalised density. Two normalisations (`full` over all obs / `active` over CI>0 obs), two color scales (`log`; `linear` rescaled per-column so col max = 1), with the sorted per-component mean CI overlaid on a twin axis `[1e-9, 1]`. |
| `plot_ci_hist_lin.py` | no | Same heatmap from the **linear-binned** `ci_hist_lin_*.npz`: y = per-token CI on a *linear* `[0, 1]` axis, 40 bins × 0.025, per-column-max linear color. `active` mode subtracts the exact-0 count from bin 0 to condition on CI>0. Reads the active shape directly without the log-axis compression against 0. |
