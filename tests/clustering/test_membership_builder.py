...
def test_membership_builder_matches_dense_thresholded_path(
    ...
    dense_activations = {
        key: torch.cat([batch_1[key], batch_2[key]], dim=0) for key in batch_1.keys()
    }
...
def test_collect_memberships_lm_all_tokens_matches_dense(monkeypatch: Any) -> None:
    ...
    def fake_component_activations(model: Any, device: torch.device | str, batch: torch.Tensor):
