...
This is completely independent of spd/harvest/. All outputs go to
spd/scripts/geometric_interaction/out/<run_id>/interaction_harvest/.
...
Examples:
    # Single GPU
    python -m spd.scripts.geometric_interaction.harvest_interaction worker \\
        --wandb_path="wandb:goodfire/spd/runs/s-55ea3f9b" --n_batches=5

    # Multi-GPU via SLURM
    python -m spd.scripts.geometric_interaction.harvest_interaction submit \\
...
) -> tuple["SPDAdapter", torch.utils.data.DataLoader[Tensor]]:
...
def worker(
    ...
    **overrides: Any,
    ...
    sampling = adapter.spd_run_info.config.sampling
...
def submit(
    ...
    **overrides: Any,
    ...
    script_module = "spd.scripts.geometric_interaction.harvest_interaction"
