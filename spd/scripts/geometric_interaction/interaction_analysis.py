...
def _title_prefix(module_name: str) -> str:
    """Return 'MLP N' for MLP up-projection modules, else the module name."""
    parts = module_name.split(".")
    if len(parts) >= 4 and parts[0] == "h" and parts[1].isdigit() and parts[2:4] == ["mlp", "c_fc"]:
        return f"MLP {parts[1]}"
    return module_name
...
def plot_heatmaps(
    ...
    for i, name in enumerate(module_names):
        ...
        images.append(im)
        ax.set_title(f"{_title_prefix(name)} {title_suffix}")
...
def plot_individual_heatmaps(
    ...
    """Save one PNG per module with its own heatmap figure."""
    individual_dir = output_dir / "individual"
    individual_dir.mkdir(parents=True, exist_ok=True)
    norm = plt.Normalize(vmin=vmin, vmax=vmax) if vmin is not None and vmax is not None else None

    for name in sorted(matrices):
        if module_filter is not None and not any(pat in name for pat in module_filter):
            continue
        mat = matrices[name].cpu().numpy()
        ...
        safe = name.replace(".", "_")
        path = individual_dir / f"{file_prefix}_{safe}.png"
...
def plot_I_distribution(
    ...
    for name in sorted(i_matrices):
        ...
        ax.set_ylabel("Count")
        ax.set_yscale("log")
        ax.set_title(f"{_title_prefix(name)} Interaction matrix off-diagonal distribution")
...
def main(config_path: Path | str | None = None, **overrides: Any) -> None:
    ...
    plot_individual_heatmaps(
        ...
        file_prefix="I",
        cmap="Purples",
        cbar_label="Interaction",
        title_suffix="Interaction matrix",
        vmin=0,
        vmax=1,
        axis_label=axis_label,
        module_filter=["mlp.c_fc"],
    )
