...
class CompressedMaskedAttention(nn.Module):
    ...
    def forward(
        self, x: Tensor, ci_q: Tensor, ci_k: Tensor, ci_v: Tensor, ci_o: Tensor
    ) -> Tensor:
        if self.n_heads == 0:
...
def blocks_replaced(
    ...
    for block, attn in attn_modules.items():
        ...
        handles.append(attn.register_forward_hook(make_attn_hook(compressed, ci_q, ci_k, ci_v, ci_o)))
    try:
        yield
    finally:
...
def main(
    ...
    params = [
        p
        for c in (*compressed_mlp.values(), *compressed_attn.values())
