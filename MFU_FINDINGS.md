...
## Overlap investigation (methodical) — runtime concurrency is the bottleneck
Per-stream xplane analysis (`scratchpad/async_fraction.py`): only **13% of gather transfers run
concurrent with compute**; **65% were on the COMPUTE stream itself** (serialized by construction).
HLO: all all-gathers ARE async-wrapped (467 start/done, 0 plain-sync) → not a cost-model decline.
Root of the stream-spill: `xla_gpu_experimental_parallel_collective_overlap_limit` **defaults to 1**
(only 1 in-flight collective; `xla_gpu_executable_num_communication_streams` defaults 0). Upstream
issue openxla/xla#14397 (remat→main-stream) is about `save_only_these_names`; our `nothing_saveable`
is the "good" policy, so NOT our cause.

A/B results (current code + 1GB):
- **OL4** (`parallel_collective_overlap_limit=4`): moved gathers OFF the compute stream (65%→**0%**)
  — but concurrency UNCHANGED (12%) and wall UNCHANGED (7.90 vs 7.88s). So stream-spill was a
  SYMPTOM; the binding constraint is prefetch **timing** (async gather-starts not hoisted to run
  during prior compute). We capture only ~21% of achievable overlap (562ms of ~2640ms).
- `num_communication_streams=4/8` (OL4S4/OL8S8): **FAILED** (flag errors on our XLA) — unusable.
- **Batch sweep b160/b192/b224: all OOM** → can't raise AI (cross the ridge) via batch without
  freeing the ~98GB activation temp first.

**Live hypothesis (Oli): unroll × overlap-limit must be COMBINED.** Each half is bottlenecked by
the other: unroll-alone exposes independent gather(L+1)/compute(L) in one body but overlap_limit=1
serializes them (→ PD_UNROLL_K measured "worse" before); overlap_limit-alone has no independent
compute in a 1-layer body to overlap against. Combined: unroll provides independent work, the limit
provides concurrency slots. Testing PD_UNROLL_K∈{2,4} × overlap_limit∈{4,8} now (concurrency = the
metric). Shippable-so-far: all_gather=1GB (−5.7%) + CI-once (−3.4%); overlap still locked.
