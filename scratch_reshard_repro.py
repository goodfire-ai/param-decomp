...
site_cs = canonical_site_cs(
    tuple(SiteC(site_name(L, k), c) for L in range(2) for k, c in REAL_C.items())
)
...
ci_arch = ChunkwiseTransformerCIArch(
    chunks=tuple(
        Chunk(input_taps=(f"resid.{L}",), output_sites=tuple(ns))
...
loss_terms = build_loss_terms(
    (
        ...
        ChunkwiseSubsetReconLossConfig(
            routing=UniformKSubsetRoutingConfig(), coeff=0.5, sites_per_chunk=7, n_samples=1
        ),
        PersistentPGDReconLossConfig(
            coeff=0.5,
            scope=SCScope(),
...
tokens = jax.device_put(
    jax.random.randint(jax.random.PRNGKey(9), (b, seq), 0, cfg.vocab_size),
