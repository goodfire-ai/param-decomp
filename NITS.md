...
## TODO (later, not now): codebase-wide dtype audit
Do a systematic pass over data types across the whole trainer. Concrete trigger: the ZeRO-1
once/step weight gather currently all-gathers **f32, not bf16** — XLA sinks the bf16 cast
*past* the gather, so we pay 2× IB traffic + 2× resident on the gathered compute-weight stack
(found while proving the ZeRO-1+TP gather-hoist mechanism, 2026-06-26). More broadly: where are
we f32 vs bf16 and is each cast placed optimally? — compute (bf16) vs SPEC-N1 fp32 masters, grad
accumulation dtype, the f32 PPGD source/Adam state, eval/KL upcasts, cast-ordering vs collectives
(the gather wart above is one instance of a general class). Likely several free memory/bandwidth
wins hiding in misplaced or unnecessary casts. Scope: a dedicated audit session, not piecemeal.
