...
Values below are from the predecessor `p-5b17949e` and are being regenerated by the fresh baseline
`p-20f9fc15` (same config, re-run on current code so it logs the `no_beta` sparsity term). The
`no_beta` row is **pending** until `p-20f9fc15` passes each mark; refresh the whole table from
`p-20f9fc15` once it does. **If you need the `no_beta` numbers before the run self-caches them**
(it's slow to reach each mark), read them directly off the W&B logs at
https://wandb.ai/goodfire/param-decomp/runs/p-20f9fc15 (metric
`ImportanceMinimalityLoss/no_beta`, at steps 20k / 50k / 400k).
...
| Importance-minimality `no_beta` (primary) | _pending_ | _pending_ | _pending_ | pure L_p sparsity term, the penalty the optimizer drives down; characterize from `p-20f9fc15` |
...
| L0 total (secondary, informational) | 1819 | 2465 | 201 | **non-monotone** (rises to 50k, then collapses 12×) — *do not* chase; informational only |
...
- **Baseline being regenerated:** `p-20f9fc15` (job 644450, 16 GPUs) re-runs the 4L config on current
  code so it logs the `no_beta` sparsity term the predecessor `p-5b17949e` lacked. It self-caches its
  own `metrics.jsonl`. **Until it passes 50k (~2.75 h), the `no_beta` bar and the trajectory table's
  `no_beta` row are pending** — fill them from `p-20f9fc15` once available. If that's still pending when
  you need the numbers, pull them straight from the W&B logs at
  https://wandb.ai/goodfire/param-decomp/runs/p-20f9fc15 (metric `ImportanceMinimalityLoss/no_beta`)
  rather than waiting for the self-cached `metrics.jsonl`. Predecessor `p-5b17949e` stays cached for the
  other metrics' reference values.
