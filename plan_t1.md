# Track T1 — confirm PD method-speedups on the *big* run, early-step screened

**Goal:** take a speedup that survived T0 (or a fresh idea) and check it on the **real 4-layer pile
run** — the one Dan actually cares about — but **judge it early** (at 20k and 50k steps) instead of
waiting for the full 400k. Same-or-better decomposition quality, faster wall-clock. One model, one
config, one baseline.

> The T1 analog of [`plan_t0.md`](plan_t0.md). T0 screens ideas cheaply on `ss-2L`; T1 confirms the
> survivors on the run that matters. `plan.md` has the fuller framing; this file is the operational
> one for T1.
...
is too slow to iterate. **Hypothesis (Dan's, likely-but-not-certain):** a variant's standing at
**20k / 50k** is indicative of where it lands at 400k. If so, we get an iteration time of **~1 h**
(20k) instead of a day.

What the baseline's own trajectory says about that hypothesis (read these as the screen's known
biases — see [`baselines.md`](track2/ledger/baselines.md)):
...
*win* on L0/PPGD still has to be confirmed at 400k (Dan's call), exactly as `plan.md` §7 says.
...
- **Config:** reproduced by `param_decomp_lab/experiments/lm/pile_llama_simple_mlp-4L.yaml` (eval
  block verified identical to the run — it's the ruler). Final 400k metrics match the older T1
  baseline `s-55ea3f9b` (PGD recon 0.660 vs 0.652, L0 201 vs 201, CE diff 0.273 vs 0.286).
...
1. **Change** one thing. Variants must reproduce `p-5b17949e` except the change — derive the config
   from `pile_llama_simple_mlp-4L.yaml` (one-line diff). Work on a branch off `feature/track2-setup`
   (never `main`); for a core edit, a worktree keeps it isolated:
   `git worktree add <path> -b feature/spd-<idea> feature/track2-setup` then `uv sync --all-packages`.
...
6. **Record** one row in `track2/ledger/README.md` + a card in `track2/ledger/experiments/<id>.md`:
   ...
   candidate** (hand to Dan), not a closed win.
...
2. **Launch one** — pick the next idea (see §Idea order), **smoke**, then **bench** vs the baseline
   config. `<10%` faster or anything fails (NaNs, tripped asserts) → record a **kill**. `≥10%` →
   submit the 50k run (`pd-lm <variant-config>.yaml --dp 16 --job_name ai-pd-lm`, background SLURM,
   default partition) and note the run_id for the next iteration to reap.

- **Idea order:** confirm **T0 survivors** first (a T0 idea that cleared its bench + quality gate on
  `ss-2L`), then draw fresh ideas from §"Where to look for speed".
...
- **GPU budget:** **≤16 GPUs per run** (`--dp 16`). Concurrent runs are fine; hundreds of GPUs total
  is OK (per `CLAUDE.md` Track-2 exception / `plan.md` §5.2).
...
- **Early ≠ converged** — 20k/50k *screen and rank*; a primary win is a **400k-confirm candidate**
  handed to Dan, not a closed result (esp. L0, which is non-monotonic — see §Why).

## Where to look for speed (measure, don't assume)

`pd-speedup-bench`'s profiler shows where the time goes. Same three compute centers as T0 (and the
T0 finding that the **PPGD inner loop dominates**, while the CI fn is a small slice, should transfer —
re-measure on the bigger 4L config, which has a much larger CI transformer: d_model 2048 / 8 blocks):

- **PPGD inner loop** — `(n_warmup_steps + n_samples)` masked component forwards
  (`metrics/persistent_pgd_recon.py`). Levers: fewer inner steps (`n_warmup_steps` — the T0 win),
  cheaper source optimizer/scope, lower precision, or an **approximate component forward** (validated
  by an equivalence test).
- **CI function** (`global_shared_transformer`, here d_model 2048 / 8 blocks) — runs every step.
  Cheaper architecture / fewer blocks / lower precision. (On T0's small CI fn this was only ~5% of
  the step and didn't clear 10% — re-check at 4L scale before assuming the same.)
