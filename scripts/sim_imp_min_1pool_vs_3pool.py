...
SITE_C: dict[str, int] = {site: SITE_C_PER_LAYER[site.split(".", 2)[2]] for site in OBSERVED_L0}
...
def gen_global_ci_uniform_binary(
    ...
    for site, C in SITE_C.items():
        ...
        out[site] = torch.bernoulli(torch.full((batch, seq, C), p_active), generator=g).float()
    return out
...
def gen_global_ci_position_concentrated(
    ...
    for site, C in SITE_C.items():
        ...
        cluster_mask = torch.bernoulli(torch.full((batch, seq, C), cluster_frac), generator=g)
        # Given in-cluster, draw Bernoulli with p_in_cluster.
        active = torch.bernoulli(torch.full((batch, seq, C), p_in_cluster), generator=g)
...
def gen_global_ci_batchwise_concentrated(
    ...
    for site, C in SITE_C.items():
        ...
        batch_use = torch.bernoulli(torch.full((batch, 1, C), active_batch_frac), generator=g)
        per_pos = torch.bernoulli(torch.full((batch, seq, C), p_when_active), generator=g)
...
def per_site_breakdown(
    ...
    *,
    ...
    for site in ci_global:
        ...
        three_pool = compute_3pool_logged(single, n_ci=n_ci, pnorm=pnorm, beta=beta, eps=eps)
...
def gen_global_ci_one_batch_element_each(
    ...
    for site, C in SITE_C.items():
        ...
        for c in range(C):
            out_t[chosen[c], :, c] = torch.bernoulli(torch.full((seq,), p_when_on), generator=g)
        out[site] = out_t
    return out
...
def main() -> None:
    print(
        f"Config: BATCH_GLOBAL={BATCH_GLOBAL}, SEQ_LEN={SEQ_LEN}, "
        f"W_DP={W_DP}, N_CI={N_CI}, p={PNORM}, β={BETA}"
    )

    run_scenario("uniform binary (iid Bernoulli per position)", gen_global_ci_uniform_binary())
    run_scenario(
        "position-concentrated (10% of positions activate components)",
        gen_global_ci_position_concentrated(cluster_frac=0.1),
    )
    run_scenario(
        "batchwise-concentrated (25% of batch elements active per comp)",
        gen_global_ci_batchwise_concentrated(active_batch_frac=0.25),
    )
    run_scenario(
        "batchwise-concentrated, more extreme (12.5% active)",
        gen_global_ci_batchwise_concentrated(active_batch_frac=0.125),
    )
    run_scenario(
        "worst-case: each component active on exactly 1 batch element",
        gen_global_ci_one_batch_element_each(),
    )
    run_scenario(
        "heavy-tail continuous (most near 0, 5% near 1)",
        gen_global_ci_heavy_tail_continuous(high_frac=0.05),
    )
    run_scenario(
        "heavy-tail continuous (most near 0, 1% near 1)",
