...
def run_grads(model, cg, idx, ci, u) -> dict[str, torch.Tensor]:
    ...
    with cg.bypass_lm_head() as lm_w, torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        (loss / n).backward()
    return {"loss": (loss / n).detach().float(), "g_V": comp.V.grad.detach().float(), "g_U": comp.U.grad.detach().float()}
...
def main() -> None:
    ...
    for k in eager:
        ...
        if a.numel() == 1:
            print(f"  {k:6s} eager={a.item():.5e} compiled={b.item():.5e} rel={abs(b.item()-a.item())/(abs(a.item())+1e-12):.2e}")
            continue
