...
TOPOLOGIES: tuple[Topology, ...] = (
    # User's stated target: 4 sites/block × DDP 4 + 2 CI + 2 PPGD = 100 GPUs.
    Topology(name="t100-s4-d4-ci2-pp2", sites_per_block=4, ddp_per_block=4, n_ci=2, n_ppgd=2),
    # Half the LW parallelism: 4 sites/block × DDP 2 + 2 CI + 2 PPGD = 52 GPUs.
    Topology(name="t52-s4-d2-ci2-pp2", sites_per_block=4, ddp_per_block=2, n_ci=2, n_ppgd=2),
    # More sites per block (8) × DDP 4 + 2 CI + 2 PPGD = 52 GPUs.
    Topology(name="t52-s8-d4-ci2-pp2", sites_per_block=8, ddp_per_block=4, n_ci=2, n_ppgd=2),
    # Smaller blocks (2 sites/block) × DDP 2 + 2 CI + 2 PPGD = 100 GPUs.
    Topology(name="t100-s2-d2-ci2-pp2", sites_per_block=2, ddp_per_block=2, n_ci=2, n_ppgd=2),
    # Maximal LW DDP, smaller blocks: 2 sites/block × DDP 4 + 4 CI + 4 PPGD = 200 GPUs.
    # NOTE: this hits 200 GPUs — uncomment only if you have headroom.
    # Topology(name="t200-s2-d4-ci4-pp4", sites_per_block=2, ddp_per_block=4, n_ci=4, n_ppgd=4),
)
...
def main() -> None:
    ...
    for topo, yaml_path in paths:
        cmd = (
            f"torchrun --standalone --nproc_per_node={topo.total_ranks} "
        ...
        r = submit_slurm_job(generate_script(cfg, cmd), f"xl-qk-{topo.name}")
        print(f"{topo.name}: job_id={r.job_id}")
