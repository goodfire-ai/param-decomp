...
def build_grad_seeds(layer_order: list[str]) -> list[torch.Tensor]:
    return [
        torch.randn(BATCH, SEQ, C_PER_SITE, device=DEVICE, dtype=torch.float32) for _ in layer_order
    ]
...
def main() -> None:
    ...
    print(
        f"  GPU bwd (events): {[f'{x:.1f}' for x in gpu_bwd_ms]}  median={sorted(gpu_bwd_ms)[2]:.1f}"
    )
