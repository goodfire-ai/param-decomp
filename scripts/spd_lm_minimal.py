...
Reproduces the method used in `spd/experiments/lm/pile_llama_simple_mlp-4L.yaml`
...
lives in `spd/pretrain/models/llama_simple_mlp.py` in the full repo.
...
class _LowerLeakyHardSigmoid(torch.autograd.Function):
    ...
    This one-way leak matches `spd/models/sigmoids.py::LowerLeakyHardSigmoidFunction` exactly.
    """
...
def _ce_next_token(logits: Tensor, input_ids: Tensor) -> float:
    ...
    so cross-batch boundaries don't contribute (matches `spd.metrics.ce_and_kl_losses`)."""
...
def eval_ce_kl_losses(
    ...
    for each. Returns 16 scalar entries matching `spd.metrics.ce_and_kl_losses`.
...
def eval_pgd_recon(
    ...
    `spd.metrics.pgd_utils._run_pgd_loop` (sign-SGD, `all_reduce(AVG)` before step, final forward).
    """
    B, S = input_ids.shape
    device = input_ids.device
...
def load_paper_target_model(
    ...
    `SPD_OUT_DIR/pretrain_cache/<project>-<run_id>/` on first download.
    """
    import types
