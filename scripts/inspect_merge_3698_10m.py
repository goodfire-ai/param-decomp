...
print("Loading 10M snapshot...", flush=True)
t0 = time.time()
snap = load_membership_snapshot(Path("/mnt/polished-lake/artifacts/mechanisms/spd/clustering/harvests/ch-2b9414a4"))
...
print(f"Snapshot loaded in {time.time()-t0:.0f}s", flush=True)

# Get group state at iter 3697
...
rank_of_selected = (cm.view(-1) < sel_cost).sum().item()
