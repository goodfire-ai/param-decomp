...
def phase_step(cg, opt, idx, totals):
    ...
    with cg.bypass_lm_head() as lm_w, torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        e = ev(); e.record(); torch.cuda.synchronize(); totals["1_target_fwd"] += s.elapsed_time(e)
        ci = torch.rand(bl, SEQ, C, device=DEV); u = torch.rand(bl, SEQ, C, device=DEV)
        mi = {SITE: ComponentsMaskInfo(component_mask=ci + (1 - ci) * u, routing_mask="all")}
        s = ev(); s.record()
        pred_h = cg(idx, mask_infos=mi)
        e = ev(); e.record(); torch.cuda.synchronize(); totals["2_masked_fwd"] += s.elapsed_time(e)
        s = ev(); s.record()
        loss, n = fused_linear_kl_div(pred_h.reshape(-1, D_MODEL), target_h.reshape(-1, D_MODEL), lm_w)
        e = ev(); e.record(); torch.cuda.synchronize(); totals["3_kl"] += s.elapsed_time(e)
        s = ev(); s.record()
        (loss / n).backward()
        e = ev(); e.record(); torch.cuda.synchronize(); totals["4_backward"] += s.elapsed_time(e)
    opt.step()
    del marks
...
def backward_only_ms(cg, opt, idx, ckpt: bool) -> float:
    ...
    with cg.bypass_lm_head() as lm_w, torch.autocast(device_type="cuda", dtype=torch.bfloat16):
        ...
        s = ev(); s.record()
        (loss / n).backward()
        e = ev(); e.record(); torch.cuda.synchronize()
    return s.elapsed_time(e)


def module_flop_buckets(cg, idx):
    with FlopCounterMode(display=False) as fc:
        with cg.bypass_lm_head(), torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            with torch.no_grad():
                cg(idx, None)
...
def main() -> None:
    ...
    print(f"  backward w/ ckpt : {bwd_ck/1000:.3f}s   w/o ckpt (grad only): {bwd_nock/1000:.3f}s")
    print(f"  => recompute ≈ {recompute/1000:.3f}s = {100*recompute/bwd_ck:.0f}% of backward; "
    ...
    if per_block:
        ...
        print(f"  per-block: {len(per_block)} blocks, ~{blk_vals[len(blk_vals)//2]/1e12:.2f} TFLOP each "
