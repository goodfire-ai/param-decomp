...
def site_breakdown_uniform() -> None:
    ...
    for site, L0, C in SITES:
        ...
        op = site_loss_uniform_per_component(L0, C, BATCH_GLOBAL // W_DP, SEQ_LEN,
                                              world_size=W_DP, beta=BETA)
    ...
    print(f"{'TOTAL':<26} {'':>7} {'':>5} {one_pool_total:>14.4g} "
...
def site_breakdown_extreme_concentration() -> None:
    ...
    element are active.
    """
    print("\nAssuming each component active on 1 of 8 batch elements "
          "(all positions in that element):")
    print(f"{'site':<26} {'L0':>7} {'C':>5} {'1pool':>14} {'3pool':>14} {'ratio':>7}")
    one_pool_total = 0.0
    three_pool_total = 0.0
    batch_local = BATCH_GLOBAL // W_DP  # 8
    ...
    for site, L0, C in SITES:
        ...
        per_rank_loss_per_active_comp = (1.0 / W_DP) * (
            ...
            + BETA * (per_comp_sum_when_on / (batch_local * seq))
...
def main() -> None:
    print(f"Config: batch_global={BATCH_GLOBAL}, seq={SEQ_LEN}, "
          f"W_DP={W_DP}, N_CI={N_CI}, β={BETA}")
    print()
    ...
    print(f"  1-pool train/loss/ImportanceMinimalityLoss ≈ 2143")
    print(f"  3-pool train/loss/imp (c4p4) ≈ 814")
    print(f"  3-pool train/loss/imp (c8p8) ≈ 735")
