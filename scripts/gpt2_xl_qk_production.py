...
def main(
    ...
    Args:
        ...
        torch_profile: When ``smoke=True``, enable torch.profiler on one rank
            per pool. Dumps Chrome trace JSON + key-averages text summary per
            rank to ``_xl_production/torch_profile/<job>/``. Heavy: ~10-50%
            slowdown on profiled ranks. Drop the json into speedscope.app.
    ...
    if smoke and torch_profile:
        ...
        env["PD_TORCH_PROFILE_RANKS"] = ",".join(str(r) for r in prof_ranks)
        env["PD_TORCH_PROFILE_OUT"] = str(tp_dir)
        print(f"torch-profile: ranks={prof_ranks} → {tp_dir} (chrome trace + key_avgs)")
