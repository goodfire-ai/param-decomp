...
def legend(
    ...
    *,
    ...
    for s_ms, e_ms, name in phases:
        ...
        lines.append(f"  c{c_start:>3} +{s_ms - origin:>5.0f}ms  {dur:>5.0f}ms  [{sync:>10}]  {name}")
    return lines
...
def render(
    ...
    *,
    ...
    out: list[str] = []
    out.append(f"step {target_step}: origin={origin:.0f}ms, span={span:.1f}ms, width={width}cols, "
    ...
    out.append("")
    # legends per rank
    out.append(f"phases ≥ {legend_threshold_ms:.0f}ms  (S = forces CPU↔GPU sync, A = async enqueue)")
