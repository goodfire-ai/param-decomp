...
# bl_ci = bl_pp = 8 (n=32 each): the CI fn (2.64B + Adam ~42GB) + target fwd + h_cache + T+1
# prefetch OOMs at bl=16, and ckpt doesn't help CI/PPGD (only LW). bl=8/n=32 is the
# lore-verified (batch-256-push-decision) fit for both pools.
N_CI = 32
N_PPGD = 32
...
def make_cfg(base: dict, comp_lr: float, ci_lr: float) -> dict:
    ...
    cfg["cadence"]["save_every"] = 5000  # normal cadence (matches _200k); XL checkpoints are large
...
def main() -> None:
    ...
    runs = {"b256_lrA_comp5e-4": (5e-4, 1e-4), "b256_lrB_comp1e-3": (1e-3, 2e-4)}
