...
def extract_pool(path: Path) -> dict:
    ...
    for e in ev:
        if e.get("cat") == "user_annotation" and _is_nccl(str(e.get("name", ""))):
            if w0 <= e["ts"] < w1:
    ...
    return {
        ...
        "nccl_by_op_ms": {k: round(v / 1000, 1) for k, v in sorted(op_time.items(), key=lambda x: -x[1])},
        "bins": bins,
    }
...
def main() -> None:
    ...
    print(f"wrote {out}")
    for p in pools:
        print(f"  {p['pool']:>9} rank{p['rank']}: wall {p['wall_ms']}ms  compute {p['compute_ms']}ms  "
