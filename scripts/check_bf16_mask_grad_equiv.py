...
def main() -> None:
    ...
    if phase == "baseline":
        ...
        ref = torch.load(out)
        report(
            "new fp32 leaf vs baseline (isolates components.py edit)",
            ref,
            lw_grads(cg, idx, ci_recv, u, delta_mask, torch.float32),
        )
        report(
            "new bf16 leaf vs baseline (full fix)",
            ref,
            lw_grads(cg, idx, ci_recv, u, delta_mask, torch.bfloat16),
        )
    else:
