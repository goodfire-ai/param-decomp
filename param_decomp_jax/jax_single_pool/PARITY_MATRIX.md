...
| **GELU variant: erf (torch) vs tanh (JAX)** | `ci_nn_blocks.py:167`; `ci_fns.py:40,68,93,129` | `ci_fn.py:93` | partial | S6 | `jax.nn.gelu` defaults to tanh; torch exact-erf. ~4.7e-4 per-element, compounds over n_blocks. **Breaks bit-level torch→JAX CI-fn transfer.** One-word fix `approximate=False`. Spec-silent; CLAUDE.md flags as "unify at run boundary". |
| **RMSNorm eps: torch dtype-finfo vs JAX 1e-5** | `ci_nn_blocks.py:174`; `ci_fns.py:229` | `ci_fn.py:58,171,186` | partial | S4 | torch fp32 finfo ~1.19e-7 vs JAX 1e-5 (~84×). Material near leaky-hard clamp boundaries (shifts the alive set). Spec-silent; CLAUDE.md deferred. |
...
| Frozen bf16 + fp32 V/U + fp32 delta upcast | `experiments/lm/run.py:117`; `components.py:71`; `vendored/.../components.py:284` | `llama8b.py:39,284,434`; `torch_config.py:237` | partial | N1/N2 | Two accepted bf16 divergences. **fp32-frozen-target requests silently downgraded to bf16** (printed warning, NOT refused) — a torch fp32-target run is NOT bit-reproducible in JAX. |
...
| CI-fn key mapping + orientation | `ci_nn_blocks.py:21-50`; `ci_fns.py:191,355` | `ci_fn.py:96-131`; `export.py:95-121` | done | — | No transpose, only permutation. **Carries the GELU + rms-eps numeric divergences** (see §4) — shifts the alive set; production-numerics divergence 0.9–18% near clamp on tiny fixtures (documented, not a mapping error). |
...
2. **CI-fn numerics (PR-8)** — GELU erf-vs-tanh and rms-eps mismatch block bit-level torch→JAX CI-fn transfer and shift the alive set. Decide: unify at a run boundary (one-word GELU fix + eps match) or codify as documented tolerance.
