...
| E7 | SLURM lifecycle: `--requeue` + `--signal=TERM@300`. A module-global SIGTERM flag is serviced at the main-loop step boundary (save-cadence OR final step OR sigterm → save then break for requeue), inside the faith-warmup loop (clean exit, no save), and inside the in-loop eval pass (abandon partial → fall through to the step-boundary save). | `run.py:216-224`, `354-359`, `388-399`; `jax_launch.py:81-91` |
| E8 | SIGTERM-save is the **low-latency fast path; periodic `save_every` is the backstop guarantee.** The flag is serviced at the train-step boundary, in the faith-warmup loop, and in the in-loop eval pass (E7), closing the two largest unserviced windows. The **first-jit-compile window remains unserviced** — a preemption there is honored only once compilation finishes and control reaches a servicing point; `save_every` covers it. (Decision recorded in SPEC S22; lore `jsp_sigterm_save_never_fired`: 6/6 historical preemptions fell back to periodic ckpts, before warmup/eval servicing.) | `run.py:216-224`, `354-359`, `388-399`; SPEC `S22`; lore `jsp_sigterm_save_never_fired` |
...
| E24 | The frozen target is stored **bf16**; V/U + CI-fn masters are fp32; AdamW + source moments are fp32 (`N1`). Each target declares the frozen-target weight dtypes it supports (`*TargetConfig.supported_weights_dtypes`); both targets are bf16-only today. A config requesting an unsupported dtype (e.g. fp32) is **refused at convert time** — no silent downgrade (issue #727). A future fp32-capable target widens its set and "just works". | `config.py` (`supported_weights_dtypes`, `build_experiment_config`); `llama8b.py:39`; SPEC `N1`, `N2` |
