...
| `slow_eval.py` | `jsp-slow-eval <run_dir> [--step N] [--no-wandb]` — JAX-native offline slow (plot) eval over a checkpoint: the `CIHistograms` / `ComponentActivationDensity` / `CIMeanPerComponent` torch metrics, rendered as PNGs (torch-free numpy/matplotlib) and logged to the run's wandb under `slow_eval/figures/*`. SimpleMLP target; no torch, no export round-trip |
| `run_state.py` | optimizer + initial-`TrainState` construction from an `ExperimentConfig` (orbax restores onto this reference) |
| `tools/` | `convert_llama_simple_mlp_checkpoint.py` (torch venv) — one-off `.pt` → safetensors conversion of the pile pretrain checkpoint |
