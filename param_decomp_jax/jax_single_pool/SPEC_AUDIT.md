...
**P3 — amend N3** to acknowledge the bf16-input intermediate asymmetry in imp-min (torch `(ci_bf16+eps)**p` bf16 intermediate then fp32 sum vs JAX cast-to-fp32-first). Document as an accepted seam, not silence.
