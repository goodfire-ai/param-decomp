...
class MetricsSink:
    ...
    def for_run(cls, run: RunInstance, is_main: bool) -> "MetricsSink":
        ...
        # The metric lists flatten into the same flat keys torch logged so cross-impl
        ...
        # `_step` axis at the eval step, so NO dedicated `slow_eval/step`
...
class BackgroundRenderer:
    ...
    `wandb.log`): the slow/plot tier (`render_and_log_slow_eval`) and the
...
def slow_eval_due(now_step: int, every: int, slow_every: int, slow_on_first_step: bool) -> bool:
    """The slow/plot tier cadence. `now_step` is always a fast-eval step (the
...
def _init_or_restore_state(
    ...
    if run.resume_provenance is not None:
        ...
        # parent↔new structural-compat check (sites + ci-fn arch) runs in the LM
