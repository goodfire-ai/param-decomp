...
def harvest_jax_run(
    ...
    server = ShardServer(
        schedule, read_dataset_meta(data.dir).seq_len, process_index=rank, process_count=world_size
    )
...
def main() -> None:
    ...
    run = open_jax_run(args.run_dir, args.step, data_root=args.data_root)
    ...
    output_dir = get_harvest_subrun_dir(args.data_root, run.run_id, subrun_id)
