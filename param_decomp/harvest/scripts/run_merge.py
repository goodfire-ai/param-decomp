...
def main(subrun_id: str, config_json: dict[str, Any], out_root: Path = DEFAULT_OUT_ROOT) -> None:
    ...
    output_dir = get_harvest_subrun_dir(Path(out_root), config.method_config.id, subrun_id)
    logger.info(f"Merging harvest results for (subrun {subrun_id})")
    merge_harvest(output_dir, config)


def get_command(subrun_id: str, config: HarvestConfig, out_root: Path) -> str:
