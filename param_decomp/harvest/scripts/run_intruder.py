...
def main(
    ...
    tokenizer_name = adapter_from_id(decomposition_id, data_root).tokenizer_name

    harvest = HarvestRepo(
        decomposition_id, subrun_id=harvest_subrun_id, readonly=True, data_root=data_root
    )
...
def get_command(
    decomposition_id: str, config: IntruderEvalConfig, harvest_subrun_id: str, data_root: Path
) -> str:
