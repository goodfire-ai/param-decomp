...
class DecompVU(eqx.Module, Generic[VULeaf]):
    ...
    def shardings(self: "DecompVU[Array]", mesh: "Mesh") -> "DecompVU[NamedSharding]":
        """Pure HSDP for the STORED masters: V `(d_in, C)` FSDP-shards d_in on `fsdp`;
        U `(C, d_out)` FSDP-shards d_out on `fsdp`. C is NEVER sharded (no TP / Megatron-C
        axis), and everything is REPLICATED over `replicate` (the across-node axis). This is
        the ZeRO-style persistence layout (master + optimizer-state memory ÷ N over the full
        mesh — `fsdp` from this shard plus `replicate` from the optimizer's ÷N reduce);
        `site_out` gathers the `fsdp`-sharded d_in/d_out for COMPUTE (its
        `with_sharding_constraint` forces the all-gather in / reduce-scatter out on `fsdp`,
        ON NVLink). The gather is bounded by `/fsdp` (8). Asserts each sharded axis tiles
        `fsdp`."""
        shard_V = NamedSharding(mesh, P("fsdp", None))  # d_in on fsdp, C replicated
        shard_U = NamedSharding(mesh, P(None, "fsdp"))  # C replicated, d_out on fsdp
        placed: dict[str, tuple[NamedSharding, NamedSharding]] = {}
        for name, (V, U) in self.vu.items():
            assert_divisible(V.shape[0], mesh, "fsdp", f"DecompVU[{name}].V.d_in")
            assert_divisible(U.shape[1], mesh, "fsdp", f"DecompVU[{name}].U.d_out")
