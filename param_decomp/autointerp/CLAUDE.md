...
```
<data_root>/runs/<spd_run_id>/autointerp/
...
- Uses OpenRouter, Anthropic, OpenAI, or Google AI (Gemini) with structured JSON outputs (`LLMConfig` in `param_decomp.autointerp.config`; provider runtime in `providers.py`)
