...
from param_decomp.autointerp.interpret import (
    resolve_target_component_keys,
    run_interpret,
)
from param_decomp.autointerp.schemas import (
    get_autointerp_dir,
...
def main(
    ...
    harvest = HarvestRepo(
        decomposition_id, subrun_id=harvest_subrun_id, readonly=False, data_root=data_root
    )
    ...
    if autointerp_subrun_id is not None:
        subrun_dir = get_autointerp_dir(data_root, decomposition_id) / autointerp_subrun_id
        ...
        subrun_dir = get_autointerp_subrun_dir(data_root, decomposition_id, autointerp_subrun_id)
        subrun_dir.mkdir(parents=True, exist_ok=True)
    ...
    adapter = adapter_from_id(decomposition_id, data_root)
