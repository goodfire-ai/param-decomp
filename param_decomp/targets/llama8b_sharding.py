...
def init_decomp_vu_placed(sites: tuple[SiteSpec, ...], key: PRNGKeyArray, mesh: Mesh) -> DecompVU:
    ...
    per_site_shardings = eqx.filter_eval_shape(partial(init_decomp_vu, sites), key).shardings(
        mesh
    )
    stacked_shardings = {
        shape: tuple(
    ...
    stacked = jax.jit(partial(init_decomp_vu_stacked, sites), out_shardings=stacked_shardings)(
        key
    )
    return jax.jit(
