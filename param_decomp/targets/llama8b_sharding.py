...
Megatron-C. The memory consumers, and how each is placed:
  ...
  * components (V/U) + their Adam states: FSDP-sharded on `fsdp` (V's d_in, U's d_out;
    C is NEVER sharded). The fp32 masters + fp32 Adam m/v are the dominant non-activation
    footprint; FSDP `/fsdp` plus the ZeRO-1 ÷N optimizer reduce splits all three over the
    full mesh.
  * CI fn + Adam states: FSDP-sharded on `fsdp` along d_model (in_proj / blocks / heads).
...
Sharding V/U FSDP keeps every einsum valid: `x @ V` gathers the `fsdp`-sharded d_in on
NVLink and contracts it; `(.) @ U` produces a `fsdp`-sharded d_out and `jax.jit` inserts the
reduce-scatter / all-reduce. No manual collectives.
