...
def main(n_layers: int, dp: int, out_path: str, seq: int = 256, batch: int | None = None) -> None:
    ...
    cfg = {
        ...
        "eval": {
            "batch_size": batch,
            "every": 100000,
            "metrics": [{"ci_alive_threshold": 0.0, "groups": None, "type": "CI_L0"}],
            "n_steps": 1,
            "slow_every": 1000000,
        ...
        "pd": {
            ...
            "ci_fn_optimizer": {
                "betas": [0.9, 0.999],
                "grad_clip_norm": None,
                "lr_schedule": {"final_val_frac": 0.1, "fn_type": "cosine", "start_val": 2.0e-05, "warmup_pct": 0.0},
                "weight_decay": 0.0,
            },
            ...
            "loss_metrics": [
                {
                    "beta": 0.2, "coeff": 5.0e-06, "eps": 1.0e-06,
                    "p_anneal_end_frac": 1.0, "p_anneal_final_p": 0.4,
                    "p_anneal_start_frac": 0.0, "pnorm": 2.0, "type": "ImportanceMinimalityLoss",
                },
                {
                    "coeff": 2.0, "n_samples": 1, "routing": {"type": "uniform_k_subset"},
                    "sites_per_chunk": sites_per_chunk, "type": "ChunkwiseSubsetReconLoss",
                },
                {
                    "coeff": 0.5, "n_samples": 1, "n_warmup_steps": 2,
                    "optimizer": {
                        "beta1": 0.01, "beta2": 0.99, "eps": 1.0e-08,
                        "lr_schedule": {"final_val_frac": 1.0, "fn_type": "constant", "start_val": 0.01, "warmup_pct": 0.025},
                        "type": "adam",
                    },
                    "scope": {"type": "per_batch_per_position"},
                    "start_frac": 0.0, "type": "PersistentPGDReconLoss",
                },
    ...
    print(f"wrote {out_path}: {n_layers}L (layers {first}..31), {len(targets)} sites, dp={dp}, batch={batch}, seq={seq}")
