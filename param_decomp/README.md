...
| `configs.py` | the torch-free pydantic config SCHEMA: routing + decomposition-target + ci-fn + loss-metric + eval-metric configs, `PDConfig` / `RuntimeConfig` / `Cadence` / `WandbConfig` / `ResumeProvenance`, and the `wandb.config` shaping helpers |
