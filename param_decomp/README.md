...
| `ci_health.py` | CI-fn training-health scalars (`eval/ci_health/*`, config-gated by the `CIFnHealth` eval metric, fast cadence): weight Frobenius/spectral norms + stable ranks (batched power iteration), instrumented-forward attention/activation stats (q/k RMS, logit growth, entropy collapse via `ChunkwiseTransformerCIFn.telemetry`), CI-logit saturation, component mode-collapse aggregates. JAX-only keys, chunkwise transformer only |
