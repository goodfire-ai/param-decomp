                    ...
                    for draw_idx, routes in enumerate(routes_per_draw):
                        ...
                        def pre_built_fwd(
                            ...
                            return masked_forward(
                                model, prepared, batch, mds[0], mds[1],
                        ...
                        with jax.named_scope("pd_recon_masked_fwd"):
                            match entry.sources:
                                case StochasticSources():  # masks built inside the target
                                    masked = batch_sharded(model.masked_output_stochastic(
                                        prepared, batch, ci_stacked, draw_key, routes,
                                        entry.live_sites, entry.has_delta,
                                        remat=remat_recon_forwards,
                                    ))
                                ...
                                case FreshPGDSources():
                                    masked = pre_built_fwd(
                                        source_masks(
                                            ci.lower, fresh_sources[(term_idx, entry_idx)], entry.live_sites
                                        )
                                    )
