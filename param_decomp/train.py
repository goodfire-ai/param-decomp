...
@dataclass(frozen=True)
class _NontargetInputs:
    """Per-step non-target batch prep for targeted PD (SPEC S34): the sharded non-target
    batch, its frozen clean output, the CI taps, and the leading shape. Built only when a
    non-target pass is configured."""
...
def make_train_step(
    ...
    *,
    ...
    `nontarget_loss_surface` (SPEC S34): when given, `step` runs the targeted-PD two-pass
    (target recon + non-target pass with the delta forced on) and takes a fifth
    `nontarget_batch` arg; when None, `step` runs the plain full-VPD single pass and ignores
    `nontarget_batch`."""
    ...
    def stochastic_entry_masks(
        ...
        force_delta_on: bool = False,
    ) -> tuple[dict[str, Array], dict[str, Array]]:
        """`force_delta_on` (targeted PD non-target pass, SPEC S35): pin every delta mask to
        1.0 so `components + Δ` reconstruct the frozen output; otherwise the delta is drawn
        `U[0,1)` like the component sources."""
        ...
        for site_idx, site in enumerate(live_sites):
            ...
            delta_masks[site] = (
                jnp.ones(leading_shape, COMPUTE_DT)
                if force_delta_on
                else random.uniform(
    ...
    def step(
        ...
        # ── shared prefix (identical to both step bodies): schedules, the sharded target
        # batch, the frozen clean target, the CI taps, and the leading shape. ──
        ...
        # Two proven bodies selected on the static `nontarget_loss_surface is None` flag: the
        # plain full-VPD single pass (numerically identical to feature/jax-arith) and the
        # targeted-PD two-pass (SPEC S34–S38). Each is a nested fn so its locals stay scoped.
        def full_vpd_body() -> tuple[TrainState, dict[str, Array]]:
            # ═══════════════ full-VPD single pass (THEIRS, verbatim) ═══════════════
            ...
            # `loss_fn` takes the live value and its ci-fn grad is pulled back through `ci_vjp`. So
            # the (≈10x-the-target) CI fn is forward-evaluated ONCE, not once detached for the
            # ascend + once inside the main backward.
            ...
            def loss_fn(
                ...
                # checkpointed block (mask never held, the memory win); others fall back to
                # building masks then `masked_output`. Either way the engine holds no per-forward
                # mask stacks.
            ...
            if ci_scaled_component_weight_decay > 0.0:
                ...
                # until `start_frac` of training via the identity keep factor. Per-subcomponent
                # max lower-leaky CI over the batch's leading dims (a collective, matching torch's
                # cross-rank MAX reduce); the CI is the pre-update forward's (stop-gradient).
        ...
        def targeted_body() -> tuple[TrainState, dict[str, Array]]:
            # ═══════════════ targeted-PD two-pass (OURS, verbatim) ═══════════════
            # tPD non-target batch prep — the clean target + CI taps for the non-target stream
            # (SPEC S34), consumed inside `loss_fn`.
            ...
            def loss_fn(
                ...
                def recon_grid(
                    ...
                    """Mean-KL recon over the term/entry/draw grid (SPEC S10'), shared by the
                    target pass and the tPD non-target pass. `force_delta_on` (S35) pins the
                    delta on and restricts the set to stochastic/constant sources; the target
                    pass (`force_delta_on=False`) additionally handles fresh-PGD / persistent
                    sources. `recon_loss` is the target's position-sliced fn (S38) or the plain
                    full-seq fn. `key_offset` keeps the two passes' per-term RNG disjoint (R1)."""
                    ...
                    for term_idx, term in enumerate(grid_terms):
                        ...
                        n_forwards = 0
                        for entry_idx, entry in enumerate(term.plan):
                            entry_key, routing_key = random.split(
                                random.fold_in(term_key, entry_idx)
                            )
                            match entry.sources:
                                case FreshPGDSources():
                                    routes_per_draw = fixed_routes[(term_idx, entry_idx)]
                            ...
                            for draw_idx, routes in enumerate(routes_per_draw):
                                ...
                                match entry.sources:
                                    ...
                                    case FreshPGDSources():
                                        assert not force_delta_on, (
                                            "non-target recon set must exclude fresh-PGD (SPEC S35)"
                                        )
                                    ...
                                    case PersistentSources(state_key=state_key):
                                        assert not force_delta_on, (
                                            "non-target recon set must exclude PPGD (SPEC S35)"
                                        )
                                        masks, delta_masks = source_masks(
                                            ci.lower,
                                            persistent_sources[state_key],
                ...
                # the SAME total so one backward grads both passes (S34). ──
                nt_aux: dict[str, Array] = {}
                if nontarget_loss_surface is not None:
                    nt_imp = nontarget_loss_surface.imp
                    nt_freq_coeff = (
                        nt_imp.cfg.frequency.coeff if nt_imp.cfg.frequency is not None else 0.0
                    )
                    with jax.named_scope("pd_ci_fn_fwd_nontarget"):
                        nt_ci = ci_batch_sharded(ci_fn_bf16(nt_inputs.taps, remat=remat_ci_fn))
                    nt_imp_lp, nt_imp_freq = imp_min_terms(nt_ci.upper, nt_imp.cfg, imp_min_param)
                    nt_total = nt_imp.coeff * nt_imp_lp + nt_freq_coeff * nt_imp_freq
                    nt_aux["nontarget/imp"] = nt_imp_lp
                    nt_aux["nontarget/freq"] = nt_imp_freq
                    nt_grid = recon_grid(
                        nontarget_loss_surface.recon,
                        nt_ci,
                        nt_inputs.batch,
            ...
            with jax.named_scope("pd_value_and_grad"):
                (
                    (
                        total_loss,
                        (faith_loss, imp_lp, imp_freq, term_losses, nt_aux, ci_wd_max),
                    ),
                    grads,
                ) = eqx.filter_value_and_grad(loss_fn, has_aux=True)(
                    (
        ...
        return full_vpd_body() if nontarget_loss_surface is None else targeted_body()
