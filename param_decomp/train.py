...
def make_train_step(
    ...
    *,
    ...
    def masked_forward(
        ...
        # the `live` sites' masked outputs, batch-sharded to match the clean targets. Passed only
        # when collecting so minimal models (without the optional param) stay callable otherwise.
    ...
    def step(
        ...
        with jax.named_scope("pd_clean_fwd"):
            # Only thread the collect sink when the aux is on (llama8b only) — models without
            # the optional param stay callable with the bare `clean_output(batch)` otherwise.
            clean_logits = (
                model.clean_output(batch, clean_site_targets)
                if clean_site_targets is not None
                else model.clean_output(batch)
            )
            clean_output = jax.lax.stop_gradient(batch_sharded(clean_logits))
        if clean_site_targets is not None:
            clean_site_targets = {
        ...
        if nontarget_loss_surface is not None:
            ...
            nt_site_targets: dict[str, Array] | None = {} if nontarget_wants_hidden_acts else None
            with jax.named_scope("pd_nt_clean_fwd"):
                nt_clean_logits = (
                    model.clean_output(nt_batch, nt_site_targets)
                    if nt_site_targets is not None
                    else model.clean_output(nt_batch)
                )
                nt_clean = jax.lax.stop_gradient(batch_sharded(nt_clean_logits))
            if nt_site_targets is not None:
                nt_site_targets = {
