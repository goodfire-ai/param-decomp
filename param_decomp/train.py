    ...
    *,
    ...
    def step(
        ...
        if mesh is not None and os.environ.get("PD_REPLICATE_WEIGHTS", "") == "1":
            ...
            full = ("replicate", "fsdp")
            components_grad = DecompVU(
                vu={
                    n: (
                        jax.lax.with_sharding_constraint(vg, NamedSharding(mesh, P(full, None))),
                        jax.lax.with_sharding_constraint(ug, NamedSharding(mesh, P(None, full))),
                    )
                    for n, (vg, ug) in components_grad.vu.items()
                }
            )
