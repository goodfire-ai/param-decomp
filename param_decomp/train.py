...
@runtime_checkable
class CollectsSiteOutputs(Protocol):
    """A `DecomposedModel` that can also emit per-site output activations from its masked and
    clean forwards — the seam the hidden-acts recon aux needs (SPEC S31). Kept off the base
    `DecomposedModel` Protocol so only targets that support it (llama8b) implement it."""
    ...
    def clean_output_and_site_outputs(self, inputs: Any) -> tuple[Any, dict[str, Array]]: ...
...
class _NontargetInputs:
    ...
    targets (SPEC S31), or None when no non-target recon term uses the aux."""
...
def make_train_step(
    ...
    *,
    ...
    use_prefix_start = isinstance(lm, SupportsPrefixResidual) and lm.split_layer > 0
    ...
    # Hidden-acts recon aux (SPEC S31): a term with `hidden_acts` set adds a site-local MSE
    # riding its masked forward, against frozen per-site targets collected from the clean
    ...
    def masked_forward(
        ...
        # shared across all forwards (the ÷N→÷fsdp gather is not re-run per forward). When
        # `collect_site_outputs` is given (hidden-acts aux), the per-`live`-site masked outputs
        # come from the SAME forward, batch-sharded to match the clean targets.
        if collect_site_outputs is None:
            logits = model.masked_output(
                prepared, batch, masks, delta_masks, routes, live_sites, has_delta,
                remat=remat_recon_forwards,
            )  # fmt: skip
            return batch_sharded(logits)
        assert isinstance(model, CollectsSiteOutputs), "hidden-acts aux needs a collecting target"
        logits, collected = model.masked_output_and_site_outputs(
            prepared, batch, masks, delta_masks, routes, live_sites, has_delta,
            remat=remat_recon_forwards,
        )  # fmt: skip
        for site, out in collected.items():
            collect_site_outputs[site] = batch_sharded(out)
        return batch_sharded(logits)
    ...
    def step(
        ...
        if use_prefix_start:
            assert isinstance(model, SupportsPrefixResidual)
            with jax.named_scope("pd_prefix_fwd"):
                batch = ResidualStart(batch_sharded(model.prefix_residual(batch)))
        ...
        def clean_forward(inputs: Any, want_targets: bool) -> tuple[Array, dict[str, Array] | None]:
            ...
            from the SAME forward (SPEC S31). Both stop-gradient'd, batch-sharded."""
            if want_targets:
                assert isinstance(model, CollectsSiteOutputs), (
                    "hidden-acts aux needs a collecting target"
                )
                logits, collected = model.clean_output_and_site_outputs(inputs)
                targets = {s: jax.lax.stop_gradient(batch_sharded(v)) for s, v in collected.items()}
            else:
                logits, targets = model.clean_output(inputs), None
            return jax.lax.stop_gradient(batch_sharded(logits)), targets

        with jax.named_scope("pd_clean_fwd"):
            clean_output, clean_site_targets = clean_forward(batch, target_wants_hidden_acts)
        ...
        # built only when a non-target pass is configured (SPEC S34), consumed inside `loss_fn`.
        nt_inputs = None
        if nontarget_loss_surface is not None:
            ...
            if use_prefix_start:
                ...
                with jax.named_scope("pd_nt_prefix_fwd"):
                    nt_batch = ResidualStart(batch_sharded(model.prefix_residual(nt_batch)))
