...
Run via `python -m param_decomp.experiments.lm.run path/to/config.yaml`.
Multi-process (DDP) entry should `torchrun` the same module.
"""
...
def build_target(target_cfg: LMTargetConfig) -> Any:
    """Load the target LM from HuggingFace, a `param_decomp.pretrain.*` wandb run, or a
...
def main(config_path: str | Path) -> None:
    ...
    eval_loader = build_lm_eval_loader(
        data_cfg,
