...
class TwoPoolTrainer:
    ...
    def snapshot(self) -> TrainingState:
        """2-pool snapshot not ported to the concept-level resumption design yet.

        See ``ThreePoolTrainer.snapshot`` for the canonical pattern; the
        equivalent for 2-pool is a straightforward port but not in scope here.
        """
        raise NotImplementedError(
            "TwoPoolTrainer.snapshot not ported to the concept-level resumption design."
        )
    ...
    def from_snapshot(
        ...
        *,
        ...
        del snapshot, target_model, run_batch, reconstruction_loss, cadence, cfg_overrides
        raise NotImplementedError(
            "TwoPoolTrainer.from_snapshot not ported to the concept-level resumption design."
        )
