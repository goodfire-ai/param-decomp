...
target/dataloaders and load the model checkpoint. Methods are recipe-mediated
through the saved recipe. Notebook callers who trained via `optimize(...)`
without a `RunConfig` should reload their checkpoint with
...
class SavedRun:
    ...
    Recipe-backed runs resolve ``run_cfg.recipe`` at construction time.
    There is no notebook-only `SavedRun`; notebook callers reload checkpoints
    via `ComponentModel.from_checkpoint(...)` directly.
    """

    path: Path
    run_cfg: RunConfig
    checkpoint_path: Path
    recipe: RunRecipe[Any]

    @classmethod
    def from_path(cls, path: ModelPath) -> "SavedRun":
        """Reload from disk or W&B. Resolves spec + checkpoint + recipe."""
    ...
    @property
    def recipe_config(self) -> BaseConfig:
        return self.run_cfg.recipe.config

    def require_recipe_config[ConfigT: BaseConfig](self, config_type: type[ConfigT]) -> ConfigT:
        cfg = self.recipe_config
        assert isinstance(cfg, config_type), (
            f"Expected recipe config {config_type.__name__}, got {type(cfg).__name__}"
        )
        return cfg

    def load_target(self) -> PDTarget:
        return self.recipe.build_target(self.recipe_config)
    ...
    def build_train_loader(
        ...
        *,
        ...
        return self.recipe.build_train_loader(
            self.recipe_config,
            device=device,
            batch_size=batch_size_override or self.run_cfg.pd.batch_size,
            seed=self.run_cfg.pd.seed,
    ...
    def build_eval_loader(
        ...
        *,
        ...
        return self.recipe.build_eval_loader(
            self.recipe_config,
            device=device,
            batch_size=batch_size_override or self.run_cfg.logging.eval_batch_size,
            seed=self.run_cfg.pd.seed,
            dist_state=dist_state,
        )
...
def load_component_model(path: ModelPath) -> ComponentModel:
    """Load a `ComponentModel` from a saved recipe-mediated PD run.
