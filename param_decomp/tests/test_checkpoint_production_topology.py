...
def _build_sharded(seed: int):
    ...
    `(replicate, fsdp)` HSDP mesh: V/U + their Adam moments FSDP-sharded over `fsdp`
    (V d_in, U d_out; C replicated), the CI fn FSDP over `fsdp` (d_model), sources + their
    Adam moments replicated, with TWO persistent terms. On the 4-device sim: `replicate=1,
    fsdp=4`; `C=8` and the V d_in / U d_out tile `fsdp`."""
    mesh = hsdp_mesh()
    cfg = _tiny_cfg()
    C, seq = 8, 16
