...
def test_eval_step_fresh_pgd_probe_device_count_invariant():
    """The fresh `c` PGD probe's KL must be invariant to device
    ...
    (torch all-reduce-AVG parity) — NOT a per-shard partial. A per-shard
    ...
    # is not achievable, but a per-shard-partial grad (the all-reduce-SUM bug) would change the
    ...
    assert abs(single_kl - sharded_kl) <= 1e-4 * abs(single_kl) + 1e-6, (
        ...
        f"sharded({n_dev}) {sharded_kl!r} — `c` source grad is not the global mean"
    )
