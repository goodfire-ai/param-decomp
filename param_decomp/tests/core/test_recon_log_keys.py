...
def test_fixed_scalar_loss_keys_use_config_type_literals():
    """`run._METRIC_KEYS` maps the two non-recon scalar terms to `train/loss/<ClassName>`;
    pin those suffixes to the config `type` literals (for `FaithfulnessLoss` that is the
    torch class name; the smooth-L0 importance-minimality term is JAX-side only)."""
