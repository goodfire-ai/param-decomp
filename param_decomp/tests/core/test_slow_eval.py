...
from param_decomp.core.slow_eval import (
    position_ci as finalize_position_ci,
)
from param_decomp.core.slow_eval import (
...
def accumulate_site_reductions(
    ...
    for residual in residual_batches:
        accumulation = fold_site_reduction(
            accumulation, step(_preactivations(model, ci_fn, residual))
        )
    return finalize_site_reductions(accumulation)
...
def accumulate_position_ci(
    ...
    for residual in residual_batches:
        accumulation = fold_position_ci(accumulation, step(_preactivations(model, ci_fn, residual)))
    return finalize_position_ci(accumulation)
