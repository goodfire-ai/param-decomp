...
These checks pin the properties that motivate it over `L_p` (see the smooth-L0 report):
flat at the origin (`phi'(0)=0`, no singularity to clip), bounded gradient
(`|phi'| <= 0.65/gamma`), redescent for clearly-on components, and the half-saturation
crossover `phi(gamma) = 1/2`.
"""
