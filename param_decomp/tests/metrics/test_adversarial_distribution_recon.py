...
def _make_state(
    ...
    *,
    ...
    return AdversaryHeadState(
        ...
        use_delta_component=use_delta_component,
        distribution=distribution,  # pyright: ignore[reportArgumentType]
        trunk_grad=trunk_grad,  # pyright: ignore[reportArgumentType]
        grl_lambda=1.0,
        head_hidden_dims=head_hidden_dims or [],
...
@pytest.mark.parametrize("distribution", ["deterministic", "gaussian_sigmoid", "beta"])
...
@pytest.mark.parametrize("distribution", ["deterministic", "gaussian_sigmoid", "beta"])
def test_grads_reach_head(distribution: str) -> None:
    """The source must carry gradient back to the head params (reparam where stochastic)."""
    model = _make_model()
...
@pytest.mark.parametrize("distribution", ["deterministic", "gaussian_sigmoid", "beta"])
def test_beta_concentration_is_bounded(distribution: str) -> None:
    """Beta alpha/beta are mapped into the stable [BETA_MIN, BETA_MAX] band even for huge logits."""
    if distribution != "beta":
        pytest.skip("bounds only apply to beta")
    from param_decomp.metrics.adversarial_distribution_recon import _BETA_MAX, _BETA_MIN

    model = _make_model()
    batch = torch.randn(4, 4)
    state = _make_state(model, distribution="beta", use_delta_component=True)
    for p in state.head.parameters():
        torch.nn.init.normal_(p, std=50.0)  # push logits to extremes
    pre_weight_acts = model(batch, cache_type="input").cache
    params, sources = state.distribution_params_and_sources(pre_weight_acts)
    for named in params.values():
        for key in ("alpha", "beta"):
            assert (named[key] >= _BETA_MIN - 1e-4).all() and (named[key] <= _BETA_MAX + 1e-4).all()
    for s in sources.values():
        assert torch.isfinite(s).all() and (s >= 0).all() and (s <= 1).all()


def test_reverse_trunk_grad_couples_into_ci_fn() -> None:
    """`trunk_grad='reverse'` routes adversary gradient into the CI trunk; `detach' does not."""
    batch = torch.randn(3, 4)

    detach_model = _make_model()
    state_d = _make_state(detach_model, distribution="deterministic", use_delta_component=True)
    _, sources = state_d.distribution_params_and_sources(
        detach_model(batch, cache_type="input").cache
    )
    sum((s**2).sum() for s in sources.values()).backward()  # pyright: ignore[reportAttributeAccessIssue]
    assert all(p.grad is None for p in detach_model.ci_fn.parameters()), (
        "detached trunk should receive no adversary gradient"
    )

    rev_model = _make_model()
    state_r = _make_state(
        rev_model, distribution="deterministic", use_delta_component=True, trunk_grad="reverse"
    )
    _, sources = state_r.distribution_params_and_sources(rev_model(batch, cache_type="input").cache)
    sum((s**2).sum() for s in sources.values()).backward()  # pyright: ignore[reportAttributeAccessIssue]
    trunk_grads = [p.grad for p in rev_model.ci_fn.parameters() if p.grad is not None]
    assert trunk_grads, "reverse mode should send gradient into the CI trunk"
    assert any((g != 0).any() for g in trunk_grads)
...
def test_trains_through_the_trainer_and_steps_the_adversary() -> None:
    ...
    assert not head_ids & {id(p) for p in trainer._ci_fn_params}
    assert not head_ids & {id(p) for p in trainer._component_params}
...
def test_double_leaky_hard_sigmoid_forward_and_directional_leak() -> None:
    from param_decomp.ci_sigmoids import double_leaky_hard_sigmoid

    alpha = 0.01
    x_vals = torch.tensor([-2.0, 0.5, 3.0])
    assert torch.allclose(double_leaky_hard_sigmoid(x_vals), torch.tensor([0.0, 0.5, 1.0]))

    # grad_output = +1 (loss wants output higher): lower rail no leak, interior passes, upper leaks.
    x = x_vals.clone().requires_grad_(True)
    double_leaky_hard_sigmoid(x).sum().backward()
    assert x.grad is not None and torch.allclose(x.grad, torch.tensor([0.0, 1.0, alpha]))

    # grad_output = -1 (loss wants output lower): lower rail leaks, interior passes, upper no leak.
    x = x_vals.clone().requires_grad_(True)
    (-double_leaky_hard_sigmoid(x)).sum().backward()
    assert x.grad is not None and torch.allclose(x.grad, torch.tensor([-alpha, -1.0, 0.0]))
