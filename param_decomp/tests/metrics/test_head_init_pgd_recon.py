...
def _cfg(**kw: object) -> HeadInitPGDReconLossConfig:
...
def _ctx(
    model: ComponentModel, batch: Tensor, *, is_eval: bool, use_delta_component: bool
) -> MetricContext:
...
def test_attack_sources_in_unit_interval(use_delta_component: bool) -> None:
    ...
    metric._ensure_state(_ctx(model, batch, is_eval=False, use_delta_component=use_delta_component))
    state = metric.state
    ...
    head_ep, random_ep, n_steps, _ = state.run_attack(
        model=model,
        batch=batch,
        ci=ci.lower_leaky,
        weight_deltas=wd,
    ...
    assert 2 <= n_steps <= 4
    assert random_ep is not None  # _cfg has random_restart=True
    exp_c = C + (1 if use_delta_component else 0)
    for ep in (head_ep, random_ep):
        for name in ("fc1", "fc2"):
            assert ep[name].shape == (5, exp_c)
            assert (ep[name] >= 0).all() and (ep[name] <= 1).all()


def test_defender_target_head_and_random_trains_ci() -> None:
    """`head_and_random` pools both PGD endpoints; the defender loss still trains the CI fn."""
    model = _make_model()
    batch = torch.randn(3, 4)
    metric = HeadInitPGDReconLoss(_cfg(defender_target="head_and_random"))
    metric.bind(model=model, device="cpu")
    ...
    assert ci_grads and any((g != 0).any() for g in ci_grads)
    metric.after_backward()  # head distill step must still work


def test_head_and_random_requires_random_restart() -> None:
    with pytest.raises(ValueError, match="random_restart"):
        _cfg(defender_target="head_and_random", random_restart=False)
...
def test_defender_loss_trains_ci_not_head() -> None:
    ...
    ci_grads = [p.grad for p in model.ci_fn.parameters() if p.grad is not None]
    assert ci_grads and any((g != 0).any() for g in ci_grads), (
        "defender loss should train the CI fn"
    )
...
def test_eval_metric_keys() -> None:
    ...
    expected = {
        ...
        f"{cls}/output_recon",
        f"{cls}/head_distill_mse",
        f"{cls}/pgd_n_steps_mean",
        f"{cls}/random_restart_win_frac",
        f"{cls}/source_frac_saturated",
    }
