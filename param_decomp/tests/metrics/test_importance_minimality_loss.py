...
class TestImportanceMinimalityLossUpdate:
    ...
    def test_compute_logs_beta_and_no_beta(self: object) -> None:
        """`compute()` emits the headline (beta-weighted) loss under the metric's own key
        and the pure L_p value — a beta-independent sparsity proxy — under a fully-qualified
        `sparsity/` key so it reads as a metric, not a loss term."""
        ...
        assert set(out) == {"ImportanceMinimalityLoss", "sparsity/ImportanceMinimalityLoss_no_beta"}
        ...
        assert torch.allclose(
            out["sparsity/ImportanceMinimalityLoss_no_beta"], torch.tensor(expected_no_beta)
        )
        assert torch.allclose(out["ImportanceMinimalityLoss"], torch.tensor(expected_with_beta))
        assert out["ImportanceMinimalityLoss"] > out["sparsity/ImportanceMinimalityLoss_no_beta"]
