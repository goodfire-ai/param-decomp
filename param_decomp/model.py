...
leading prefix. The three model edges are generic too — the model's
...
class DecomposedModel(Protocol):
    ...
    `sites` fixes the canonical site order — chunking and the CI fn's
    ...
    minimizes: the LM uses `kl_per_position`; a non-LM target supplies its own
    ...
    def recon_loss_fn(self, masked_output: Any, clean_output: Any) -> Float[Array, ""]:
        """Recon comparison the step minimizes. LM: `kl_per_position`."""
        ...

    def clean_output(self, inputs: Any, /) -> Any:
        """All-frozen forward — the recon target; never the `mask=1` decomposed
    ...
    def clean_output_and_activations(
        ...
        """`(clean_output(inputs), read_activations(inputs, wanted))` — semantics
    ...
    def prepare_compute_weights(self, vu: ComponentStacks) -> Any:
        ...
        (semantics unchanged — a read-only compute view). Mask-INDEPENDENT, so the result is shared
    ...
    def masked_output(
        ...
        *,
        ...
        """The masked decomposed forward. `prepared` is the output of
        ...
        whose delta mask is a constant 0. `remat` (static) gates
    ...
    def masked_site_outputs(
        ...
        (`((x@V)*m)@U + (x@Δ)*d`), keyed by site. `prepared` is the output of
...
def stochastic_site_masks(
    ...
    from `draw_key`. The generic stochastic-source formula, shared by the
...
def chunk_sites(site_names: tuple[str, ...], sites_per_chunk: int) -> tuple[tuple[str, ...], ...]:
    """Sequential `sites_per_chunk`-groups in the canonical site order."""
