...
| S34 | **Stale-CI replay** (config-gated: `DataConfig.replay_ci_update ∈ {every, first, last, mean}`, default `every`; the non-`every` modes require `train_batch_replay > 1`; **ADDED 2026-07-06, AMENDED 2026-07-07** (mode knob replaces the `replay_stale_ci` bool: `first` = the old `true`), experimental — an S4-cadence amendment riding the free-AT batch replay): within a replay window the taps are BIT-IDENTICAL across steps (a pure fn of the frozen target + the shared batch) and the ci_fn params are held between its once-per-window updates, so ONE CI envelope per window is exact regardless of which window step computes it. `every` emits the pre-S34 step unchanged (live CI, ci_fn update every step). The non-`every` modes update the ci_fn ONCE per window (matching the 1-per-window unique-data rate) and on every NON-updating step reuse the window's envelope as a CONSTANT — no taps read, no CI-fn forward or backward, no ci_fn/optimizer update; V/U, faithfulness, and both adversaries behave exactly as S12′–S14′ with `ci_lower` a constant (their gradients are unchanged by CI being live vs constant); the imp-min term is computed from the cached upper for logging. Per mode, with `k = train_batch_replay` and window steps `t = 0..k-1`: **`first`** — `t=0` runs the full step (taps → CI-fn forward/vjp inside the main backward → ci_fn update) and its envelope seeds the window; `t=1..k-1` are the constant-envelope repeat steps (envelope cotangent discarded — zero ci_fn gradient there). **`last`** — `t=0` computes the envelope by a plain CI-fn forward (`compute_ci`: no vjp, no update) and runs a repeat step; `t=1..k-2` are repeat steps; `t=k-1` runs the full step with live CI + the ci_fn update — its own CI forward reproduces the cached envelope (taps + params window-constant), so the ONLY semantic difference vs `first` is WHERE the one-per-window ci_fn gradient is taken: against the window-END adversary/component state. **`mean`** — every window step runs a repeat-style step whose main loss takes the envelope as a TRACED input: the same backward that yields the component/source grads also yields the cotangent `∂L_t/∂envelope` (both `.lower` via the masks and `.upper` via imp-min; the adversary ascents still see the envelope DETACHED, exactly like the full step's `ci_lower_detached`, §4.5). The per-step cotangents accumulate in fp32 across the window; at `t=k-1` the step adds its own cotangent, divides by k, pulls the mean back through ONE CI-fn vjp to a ci_fn param gradient, and applies ONE Adam update — EXACTLY the mean of the k per-step ci_fn gradients `every` would produce with the ci_fn frozen within the window (the vjp is the same linear map at every window step: params + taps window-constant; means and pullbacks commute), no approximation. In all non-`every` modes the ci_fn LR follows the run's GLOBAL-step schedule via a unit-LR optimizer + in-step `scheduled_value_traced` at the updating step's global step (`run_state.unit_lr_ci_fn_optimizer`; optax's update count advances once per window, so a count-driven schedule would stretch by the replay factor); Adam bias correction correctly tracks the update count. The `grad_norms/…ci_fns` metric family is emitted only on ci_fn-updating steps. A requeue-resume landing MID-window rebuilds the envelope from the current ci_fn (`StaleCITrainSteps.compute_ci`; under `first` that envelope is one ci_fn Adam step fresher than the interrupted window's) and, under `mean`, drops the partial cotangent accumulator — the window-last mean then covers only the steps since resume. Tiny, preemption-only trajectory wobbles (weaker than S22's exactness; accepted for the experimental knob). |
