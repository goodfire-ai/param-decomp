...
| S20 | Both main optimizers are AdamW, `wd=0`, betas `(0.9, 0.999)`, eps `1e-8`; LR cosine to `0.1×` start, no warmup, stepped per training step. Cosine convention is canonical-torch: `progress = (step − warmup) / (decay_steps − 1)`, so LR reaches `0.1×` at `step = steps − 1` (`schedule.py`). This is matched JAX-side (#642); plain `optax.cosine_decay_schedule(lr, steps, alpha=0.1)` divides by `steps` and reaches `0.1×` one update LATER (at `count = steps`), a genuine formula divergence of ~O(1/steps) ≈ 2.5e-6/step at 400k that the JAX schedule must avoid by using the `decay_steps − 1` denominator. Both main LRs are evaluated by `scheduled_value_traced` honoring the full `ScheduleConfig`; the canonical cosine-to-0.1-no-warmup shape is pinned by the lab conversion gate (`assert_canonical_algorithm_config`). **AMENDED 2026-07-14** (Oli-requested, experimental): the components optimizer is config-gated — `ComponentNormOptimizerConfig` (`type: component_norm`, `param_decomp/component_norm.py`) selects a Muon-style per-subcomponent normalized SGD for the V/U masters instead: optional S19-style global clip (default off; ~inert through the normalization, exists to match muon-arm clip semantics in controlled comparisons) → optional Nesterov momentum → each component's `(gV_c, gU_c)` divided by `max(‖dW_c‖ + eps, norm_floor)` where `‖dW_c‖` is the frobenius/spectral norm of the induced rank-≤2 weight update `dW_c = gV_c ⊗ U_c + V_c ⊗ gU_c` → scheduled LR; no pinned schedule shape. `NesterovSGDOptimizerConfig` (`type: nesterov_sgd`) is the same chain minus the normalization stage (the control isolating momentum/schedule/clip from per-slice normalization). S19/S20 as written (AdamW + clip 0.01 + cosine-to-0.1) remain the CANONICAL method and the default `type: adam` path; the CI-fn optimizer is always AdamW per S20. |
