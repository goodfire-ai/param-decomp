...
| S34 | **Mixed per-sample persistent+stochastic sources** (`mixed_persistent_stochastic(state_key, adv_fraction)`; config `MergedStochasticPGDReconLossConfig`, strategy `MixedPersistentStochasticSources`. **ADDED 2026-07-16** (#6)). Per forward, ONE `assign ~ Bernoulli(adv_fraction)` draw per BATCH ELEMENT, broadcast over all position axes — deliberately NOT per-position (this is the "distribution as stated" for R1 purposes: the assignment bit is per-sample by design, so no sample's loss is ever scored against a mixed-family attention context). Where `assign`: the term's persistent bundle supplies both the component sources and the delta channel, and that sample's routing is forced all-live. Where `¬assign`: fresh `U[0,1]` sources per (position, component[+delta]), routed per the entry's `SAMPLE_ROUTING` draw. Masks compose per S1 from the selected family; the selection is a mask-level `where`, which also GATES the source gradient — the persistent leaves receive gradient only from adversarial-assigned samples, so the final ascent's per-step coverage is the `adv_fraction` sample subset (S24's route-all / all-position warmup ascents are unaffected and remain the full-coverage backstop). This is the ONE mask source that modulates its entry's routing (`route = assign OR routing_draw`); `SAMPLE_ROUTING` semantics are otherwise untouched. A term carrying this strategy IS a persistent term for every persistent-source invariant: S12′ (sources enter the loss forward as leaves), S13′ (`n_warmup + 1` updates through its own SRC_STEP; constant-only source LR), S14′ (final ascent from the main backward, unscaled by the term's coeff), S15 (PROJ after every update), S16 (scope/replica semantics), S22 (sources + SRC_STEP moments checkpointed), S23 (its bundle feeds this one term), S24 (warmup ascents route-all). Expectation identity: conditional on state, `E_assign[term] = adv_fraction · (all-sites persistent term) + (1 − adv_fraction) · (stochastic term)` — EXACT, because batch elements are independent in the forward, so each sample is a pure-family sample of the corresponding standalone term (the per-sample draw's whole point; a per-position draw would couple families through attention). So one term at `coeff 1.0, adv_fraction 0.5` replaces the canonical `0.5·stoch + 0.5·persistent` pair in expectation, in one masked forward. The semantic delta vs the standalone pair is adversary coverage/staleness only: each persistent slot updates from the ~`adv_fraction` of steps its sample draws adversarial. |
...
| `MASK_SOURCE` | `stochastic` (fresh U[0,1]/Bernoulli per draw) · `constant(v)` (`v=0` CI-masked, `v=1` unmasked; no delta path) · `fresh_pgd(init, n_steps, step_size, scope)` · `persistent(state_key)` · `mixed_persistent_stochastic(state_key, adv_fraction)` (the merged stochastic+persistent term, `MergedStochasticPGDReconLoss`: one Bernoulli(`adv_fraction`) bit per BATCH ELEMENT selects that whole sample's family — the persistent bundle routed all-live, or fresh U[0,1] routed per the entry's `SAMPLE_ROUTING`; full contract in S34. **ADDED 2026-07-16** (#6)) | ★ stochastic + persistent |
