...
| S24 | A persistent term draws its routing ONCE per step from ITS routing config, shared by ALL its warmup ascents and its main loss forward — the adversary trains and is scored under the same routing. **AMENDED 2026-07-13** (Oli, slack): supersedes the former warmup route-all torch-parity pin — for route-all configs (every run before 2026-07-13) the behavior is byte-identical. A fresh-PGD entry likewise draws its routing ONCE per step, shared by all its ascents and its main loss forward (torch parity: `pgd_masked_recon_loss_update`). |
