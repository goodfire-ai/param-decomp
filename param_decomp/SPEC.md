...
| S3 | The recon target `clean_output` is the frozen-path forward (the full model: embed the token batch, all blocks, final norm, LM head), stop-gradient. Never the `mask=1, delta=1` decomposed identity (differs in bf16 and pollutes the graph). A subset decomposition simply leaves non-decomposed blocks on the frozen `x @ W` path. **AMENDED 2026-06-24** (Oli-approved): removed residual-start — the model takes the token batch and embeds internally, so `clean_output` IS the whole-model frozen forward (was: the suffix-only forward over a separately-harvested residual, argued equal to the whole-model forward under sg of a frozen prefix). See REMOVE_RESIDUAL_START_DESIGN.md. **AMENDED 2026-07-13** (pending Oli sign-off): prefix REUSE restored as a step-internal optimization, per that design doc's own ad-hoc seam. The model interface is unchanged (token inputs; `clean_output` is still the whole-model frozen forward), but a target whose decomposed sites all live past a frozen lead (`split_layer > 0`) exposes `prefix_residual(tokens)` (stop-gradient, `lm.py::SupportsPrefixResidual`), and the train step computes it ONCE per stream and passes a `ResidualStart` to every forward — equal to the token forward by the same sg-of-a-frozen-prefix argument as the original design (verified: `tests/test_prefix_reuse.py`). Eval/consumer paths keep token inputs. |
...
| S18 | Each training step consumes a fresh token batch (a pure function of `(seed, step)` for O(1) resume); the model embeds it internally. **AMENDED 2026-06-24** (Oli-approved): removed the prefix harvest (residual-start) — there is no separate prefix forward; `clean_output` / `read_activations` / `masked_output` take the token batch directly. **AMENDED 2026-07-13** (pending Oli sign-off): the step may substitute a `ResidualStart` (the once-per-stream stop-gradient prefix residual) for the token batch on all its forwards when `split_layer > 0` — an in-graph reuse, not a separate harvest job; the batch schedule stays a pure function of `(seed, step)` over TOKENS. |
