...
class SmoothL0ImportanceMinimalityLoss(Metric[SmoothL0ImportanceMinimalityLossConfig]):
    ...
    def compute(self) -> MetricResult:
        ...
        # `smooth_l0` is the beta-independent sparsity proxy (≈ active-component count),
        # not a loss term — emit it under the `sparsity/` namespace (fully-qualified key)
        # so it doesn't read as a loss.
        return {
            name: smooth_l0 + self.cfg.beta * entropy,
            f"sparsity/{name}_no_beta": smooth_l0,
        }
