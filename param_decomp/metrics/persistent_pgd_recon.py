...
class _PersistentPGDReconBase[
    ...
    def state_dict(self) -> dict[str, Any]:
        if self.state is None:
            # No persistent state yet — either we were never updated, or update() was gated
            # off by `start_frac` and never created the state machine.
            return {}
    ...
    def load_state_dict(self, state: dict[str, Any]) -> None:
        if not state:
            # Saved-from-uninitialised: nothing to restore.
            self._pending_resume_state = None
            return
        if self.state is None:
