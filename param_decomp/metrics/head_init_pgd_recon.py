...
class HeadInitPGDReconLossConfig(LossMetricConfig):
    ...
    def _validate_steps(self) -> "HeadInitPGDReconLossConfig":
...
class HeadInitPGDState:
    ...
    def __init__(
        ...
        *,
        ...
        self.head = _build_head(
            d_model, cfg.head_hidden_dims, sum(self._source_c.values())
        ).to(device)
    ...
    def predict_sources(self, pre_weight_acts: dict[str, Float[Tensor, "... d_in"]]) -> AdversarySources:
    ...
    def run_attack(
        ...
        *,
    ...
    ) -> tuple[AdversarySources, int, bool]:
        """Refine the head init with sign-PGD (random step count); optional random restart.

        Returns `(s_k_detached, n_steps_used, restart_won)`. CI is detached inside PGD so the
        attack optimizes only the sources. `restart_won` is whether the random-restart endpoint
        beat the head-initialized one (a diagnostic for how good the head's init is).
        """
        ...
        s_k, loss_head = _refine_pgd(init_sources=head_sources, **refine_kwargs)  # pyright: ignore[reportArgumentType]
        restart_won = False
        if self._cfg.random_restart:
            random_init = {
                name: torch.rand_like(src) for name, src in head_sources.items()
            }
            s_k_rand, loss_rand = _refine_pgd(init_sources=random_init, **refine_kwargs)  # pyright: ignore[reportArgumentType]
            if loss_rand.item() > loss_head.item():
                s_k, restart_won = s_k_rand, True
        return s_k, n_steps, restart_won

    def distill_loss(self, head_sources: AdversarySources, target: AdversarySources) -> Float[Tensor, ""]:
...
class HeadInitPGDReconLoss(Metric[HeadInitPGDReconLossConfig]):
    ...
    def update(self, ctx: MetricContext) -> Tensor | None:
        ...
        s_k, n_steps, restart_won = self.state.run_attack(
            model=self.model,
            batch=ctx.batch,
        ...
        # Head distillation loss (stepped from after_backward on a graph disjoint from total_loss).
        distill = self.state.distill_loss(head_sources, s_k)
        self._pending_distill = distill if not ctx.is_eval else None

        # Defender loss: recon at the detached PGD endpoint (trains components + CI fn via `ci`).
        ...
        loss, n = ctx.reconstruction_loss(pred=out, target=ctx.target_out)

        if ctx.is_eval:
            self._recon_sum += loss.detach()
            self._recon_n += n
            self._accum_eval(ctx, s_k, distill.detach(), n_steps, restart_won, weight_deltas)
