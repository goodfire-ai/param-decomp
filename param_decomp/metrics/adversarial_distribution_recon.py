...
- `deterministic`: `source = double_leaky_hard_sigmoid(logit)` (no sampling) — the clean
  baseline isolating "does the *distribution* help" from the trunk/head architecture.
...
class AdversaryHeadState:
    ...
    def _sample(
        ...
        match self._distribution:
            case "deterministic":
                logit = params[..., 0]
                return double_leaky_hard_sigmoid(logit), {"logit": logit}
