...
def launch_layerwise_run(
    ...
    wandb_group = orchestrator.wandb_group
    assert wandb_group is not None  # narrows for type checker; enforced by validator
    ...
    per_module_configs = _split_per_module(orchestrator)
...
def _split_per_module(orchestrator: Config) -> list[Config]:
    return [
        orchestrator.model_copy(
            update={
                "module_info": [info],
                "wandb_run_name_prefix": f"{orchestrator.wandb_group}-{info.module_pattern}-",
            }
        )
