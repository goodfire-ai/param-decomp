...
class CIBlock(eqx.Module):
    ...
    def shardings(self, mesh: Mesh) -> "CIBlock":
        ...
        assert self.wo.shape[1] % n_data == 0, f"CIBlock wo d_model {self.wo.shape[1]} not ÷ {n_data}"
        assert self.w1.shape[2] % n_full == 0, f"CIBlock w1 mlp_hidden {self.w1.shape[2]} not ÷ N={n_full}"
        assert self.w2.shape[1] % n_full == 0, f"CIBlock w2 mlp_hidden {self.w2.shape[1]} not ÷ N={n_full}"
        return eqx.tree_at(
...
class ChunkTransformer(eqx.Module):
    ...
    def shardings(self, mesh: Mesh) -> "ChunkTransformer":
        ...
        data = ("replicate", "fsdp")
        in_proj_sh = NamedSharding(mesh, P(None, "tp", data))  # in_proj: total_d_in ÷tp, d_model ÷(rep·fsdp) → ÷N (row-parallel)
        out_ws_sh = NamedSharding(mesh, P(None, data, "tp"))  # out_ws: d_model ÷(rep·fsdp), C ÷tp → ÷N
        repl = NamedSharding(mesh, P())
        n_data = mesh.shape["replicate"] * mesh.shape["fsdp"]
        n_tp = mesh.shape["tp"]
