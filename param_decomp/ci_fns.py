...
class GlobalSharedTransformerCiConfig(BaseConfig):
    ...
    down_proj_identity_init: bool = Field(
        default=False,
        description="If True, initialise each per-layer SwiGLU down-projection block as an "
        "identity matrix (so the head starts as a pass-through of the SwiGLU neurons). "
        "Only applies to fn_type='global_shared_swiglu'.",
    )
    ...
    def validate_ci_config(self) -> Self:
        match self.fn_type:
            ...
            case "global_shared_transformer" | "global_shared_swiglu":
                ...
                if self.fn_type == "global_shared_transformer":
                    assert not self.simple_transformer_ci_cfg.down_proj_identity_init, (
                        "down_proj_identity_init only applies to fn_type='global_shared_swiglu'"
                    )
        return self
...
class GlobalSharedSwigluCiFn(nn.Module):
    ...
    With `down_proj_identity_init`, each block starts as an identity matrix so the head
    initially passes the SwiGLU neurons through unchanged. Layer order is fixed by sorted
    layer name so concatenation is deterministic. Only linear (non-embedding) targets are
    supported.
    ...
    def __init__(
        ...
        down_projections = {
            name: Linear(C, C, nonlinearity="linear")
            for name, C in zip(self.layer_order, self.c_split_sizes, strict=True)
        }
        if down_proj_identity_init:
            with torch.no_grad():
                for down_proj in down_projections.values():
                    down_proj.W.copy_(torch.eye(down_proj.input_dim))
        self._down_projections = nn.ModuleDict(
            {name.replace(".", "-"): dp for name, dp in down_projections.items()}
        )
    ...
    match ci_fn_type:
        ...
        case "global_shared_swiglu":
            ...
            return GlobalSharedSwigluCiFn(
                ...
                n_layers=transformer_cfg.n_blocks,
                attention=_attention_from_cfg(transformer_cfg.attn_config),
                down_proj_identity_init=transformer_cfg.down_proj_identity_init,
                mlp_hidden_dims=transformer_cfg.mlp_hidden_dim,
            )
