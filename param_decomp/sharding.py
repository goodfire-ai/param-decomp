...
from param_decomp.configs import GPUS_PER_NODE


def init_distributed(dp: int) -> None:
    """The `launch: slurm` process bring-up: `jax.distributed` over `dp // 8` nodes.
    Distributedness is config-driven (`runtime.launch`), NEVER inferred from ambient SLURM
    env — `SLURM_PROCID` is present in every process on a SLURM box (incl. a pytest
...
def _hsdp_shape(n_devices: int, tp: int) -> tuple[int, int, int]:
    """`(replicate, fsdp, tp)` for a world of `n_devices` — the ONE shape rule both the
    concrete and abstract mesh constructors share. A multiple-of-8 world splits into
    8-GPU in-node blocks; a smaller world (an inline sub-node run, or CPU sim at a
    non-multiple-of-8 count) IS one in-node block (so the divisibility asserts still bite
    on the real shard dims)."""
...
def hsdp_mesh(tp: int = 1) -> Mesh:
    ...
    replicate, fsdp, tp = _hsdp_shape(devices.size, tp)
    return Mesh(devices.reshape(replicate, fsdp, tp), axis_names=("replicate", "fsdp", "tp"))


def hsdp_abstract_mesh(dp: int, tp: int) -> AbstractMesh:
    ...
    return AbstractMesh(_hsdp_shape(dp, tp), ("replicate", "fsdp", "tp"))
