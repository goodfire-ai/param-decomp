...
| S34 | `mixed_persistent_stochastic(state_key, adv_fraction)` (`MergedStochasticSubsetPPGDReconLossConfig` / `MixedPersistentStochasticSources`, #6): per forward, ONE `assign ~ Bernoulli(adv_fraction)` draw per BATCH ELEMENT (`adv_fraction` is a `ScheduleConfig` evaluated per step like S9's pnorm; both endpoints must lie in [0, 1]), broadcast over all position axes — per-sample by design (the R1 "distribution as stated"), so no sample is scored against a mixed-family attention context. `assign` → the term's persistent bundle (component sources + delta channel), routing forced all-live; `¬assign` → fresh `U[0,1]` per (position, component[+delta]), routed per the entry's `SAMPLE_ROUTING`. Masks compose per S1; the mask-level `where` gates the source gradient to adversarial-assigned samples (S24's route-all warmup ascents remain the full-coverage backstop). This is the one mask source that modulates its entry's routing (`route = assign OR routing_draw`). A term carrying it IS a persistent term for S12′–S16 and S22–S24. Expectation identity, exact since batch elements are forward-independent: `E_assign[term] = adv_fraction · (all-sites persistent term) + (1 − adv_fraction) · (stochastic term)` — so `coeff 1.0, adv_fraction 0.5` replaces the canonical `0.5·stoch + 0.5·persistent` pair in one masked forward; the semantic delta vs the pair is adversary coverage/staleness only (each persistent slot updates on the ~`adv_fraction` of steps its sample draws adversarial). |
| S35 | **Block-residual relative-MSE auxiliary training loss** (`ResidualMSELossMixin.residual_mse_coeff`, Lucius-requested; an explicit training-loss amendment per S31's own "keep-on-bridge unless a training-loss use case appears" clause — a DIFFERENT seam from S31's, not a reversal of it). Any recon-loss config carrying the mixin (`StochasticRecon{,Layerwise,Subset}Loss`, `ChunkwiseSubsetReconLoss`, `PGDRecon{,Layerwise,Subset}Loss`, `PersistentPGDReconLoss`, `MergedStochasticSubsetPPGDReconLoss` — NOT the constant-source floor/ceiling probes `CIMaskedRecon*`/`UnmaskedRecon`) may set `residual_mse_coeff: float \| None`; when set, every draw's objective becomes `recon_loss_fn(masked, clean) + residual_mse_coeff * residual_mse_loss(masked_block_resids, clean_block_resids)` (`train.py::combined_objective`) BEFORE the term's own `coeff` scales the sum — so this coefficient is relative to the e2e loss, not the trainer's total. `residual_mse_loss` (`losses.py`) is `mean_block [ Σ(masked−clean)² / Σ(clean²) ]` (the inner sums over every axis but the block axis — batch[, sequence] × d — via `relative_mse_per_block`, a RELATIVE per-block MSE guarding against activation-scale differences across blocks/models), AVERAGED (not summed) across blocks — **AMENDED (Lucius-requested, 2026-07-23):** originally summed, matching the compression-project precedent (`run_mse.py`, relative MSE summed over per-block points), but a sum makes `residual_mse_coeff`'s effective strength scale with block count, forcing per-depth re-tuning (Lucius's own "25 = 100 ÷ 4 layers" starting guess was manually compensating for exactly this); averaging makes the coefficient depth-invariant, so it now means "weight on the per-block-averaged relative MSE" directly. Taken specifically at the residual stream immediately after every transformer block's full attn+mlp update (deliberately NOT at every decomposed site/weight-matrix output the way S31 is — reconstructing every intermediate would refit noise the softmaxes/nonlinearities mostly filter out; the block boundary is the intended granularity) — distinguishing this from S31 in BOTH seam (fused into the training forward, not a separate offline eval forward) and tap granularity (block boundary, not per-site). The residual taps ride the SAME forward the e2e loss already runs — no extra forward pass (the efficiency property S31's per-site variant lacks): a narrowed `ResidualStreamModel(DecomposedModel, Protocol)` (`model.py`, checked via `isinstance` like `ComponentActivationModel`/`AttnPatternModel` — transformer targets only, TMS/ResidMLP have no block structure to tap) adds `clean_output_with_block_resids` / `masked_output_with_block_resids` / `masked_output_stochastic_with_block_resids`, each returning `(same_output, block_resids)` where `block_resids[layer]` is `[n_block, *leading, d]` — implemented by extending the EXISTING scan/loop bodies (`GLUDecomposedModel._run_masked_forward` / `SimpleMLPDecomposedModel._run_masked_forward`, both frozen AND live segments, since S35 needs every block's residual, not just live-site ones) to optionally also emit each block's post-attn+mlp residual as a scan/loop aux output, gated by a static `collect_block_resids` bool — zero cost when false (retrace-time branch, no runtime `cond`). `train.py::make_train_step` computes `clean_resids` ONCE per step (alongside `clean_output`, reused by every term that wants it) and threads `collect_resids` per-(term,entry) through `masked_forward`/`stochastic_forward` so only entries whose TERM configured `residual_mse_coeff` pay the extra collection. For the adversarial recon losses, the adversary ascends the SAME `combined_objective` used by the main loss, not the bare e2e loss — for `PersistentPGDReconLoss`/`MergedStochasticSubsetPPGDReconLoss`'s FINAL ascent (S13'/S14') this falls out for free (the final ascent reuses the main backward's source-grad, and the main backward now differentiates the combined objective when the term's `residual_mse_coeff` is set); the WARMUP ascent (S24, a separate pre-backward scoring forward shared by all persistent adversaries) is NOT term-specific machinery, so `make_train_step` builds one `make_warmup_scoring_loss`-derived closure PER adversary (`state_key`), closing over that state_key's OWNING term's `residual_mse_coeff` (S23: a source bundle feeds exactly one term, so this lookup is unambiguous) — `persistent_residual_mse_by_key` in `train.py`. Fresh-PGD's ascent (`entry_loss_for_sources`) takes the same `residual_mse_coeff` from its entry's term directly (no adversary/state_key indirection needed — fresh sources aren't persisted). **Known cost not yet optimized**: this materializes BOTH full `[n_block, *leading, d]` residual stacks (masked + clean) as ordinary scan/loop outputs rather than reducing the relative-MSE sum inline per-block inside the forward — a real, bounded memory addition (roughly one extra unrolled-model's worth of `d`-wide activations) proportional to how many recon-loss draws configure it. Acceptable for the requested use case; if this becomes a bottleneck at production scale, an inline-accumulate-then-discard variant (fusing the clean+masked forwards into one dual-carry scan) would remove it — deferred, not built, since the straightforward version was correct and simple first. **Logging (Lucius-requested, 2026-07-22):** `loss/<name>` stays the combined training objective, but each configured term ALSO logs `loss/<name>/e2e` (the bare `recon_loss_fn` component, always present) and `loss/<name>/residual_mse/block_{i}` for `i` in `range(n_block)` (only when `residual_mse_coeff` is set) — mean-over-draws of the same quantities `train.py::objective_with_breakdown` sums into the combined loss, so a run can show which piece is actually moving. |
...
| D4 | Validation property: with global batch + seed fixed, the metric trajectory is invariant to device count up to floating-point reassociation (cross-shard reduction order; observed rel ≤ ~1e-5 on the tiny-target harness, `param_decomp/targets/invariance_check.py`). JAX's counter-based RNG makes even the stochastic draws identical across layouts. **AMENDED 2026-07-15** (Oli-requested, owner-partitioned persistence): the trainable V/U masters (and their optimizer moments) persist as same-shape STACKS — one `(Vs [g, d_in, C], Us [g, C, d_out])` pair per shape group (`components.ComponentStacks.stacks` + static `site_slots`), placed by the hybrid HSDP rule: stack axis ÷`replicate` (whole matrices owned per node-group — zero cross-node weight collectives per step; muon NS node-local), matrix d dims ÷`fsdp`, C ÷`tp` (total ÷N, same memory as the retired per-site intra-matrix ZeRO-1); a group whose stack length does not tile `replicate` falls back per-group to intra-matrix data sharding behind the stack axis. Same math, different layout — covered by THIS invariant's reassociation tolerance (re-validated on the harness at 4 sim devices). Checkpoint trees change shape (2·n_shapes stack leaves, not 2·n_sites): pre-change checkpoints need a one-off layout migration to restore at tip. Muon leaf labeling: the V/U tree is now all-3D, so both groups use `stacked_muon_dimension_numbers` (optax's default 2D rule would silently Adam every V/U leaf). **AMENDED 2026-07-21** (Oli-approved): the non-tiling-group fallback is config-OPT-IN, not automatic — enabled by the `owner+zero1` preset (or an explicit table's declared `params.zero1` row); strict `owner` errors on a group whose stack length does not tile `replicate`. **AMENDED 2026-07-21** (Oli-approved, same day): the sharding preset/table is a BIDIRECTIONAL claim, checked at config build — the per-group persist-vs-zero1 assignment is resolved once, at `PlacementRules` construction from the run's resolved site set and its config-implied mesh (for a `dp: N` launch this fires at `pd-lm` submit validation, on the login node before sbatch); a declared `params.zero1` row that no shape group takes is equally an error, and the trainer boundary only validates the received assignment against the arrays it holds — it never re-decides. |
...
n-pool lineage (`param_decomp/metrics/chunkwise_subset_recon.py`) — there is no
...
| §4.3 recon term (stochastic) | OFF-BRANCH: `param_decomp/metrics/chunkwise_subset_recon.py` (`ChunkwiseSubsetReconLoss`) on `feature/fsdp-lm-trainer`; pinned by `tests/equivalence/` fixtures, no on-branch Metric |
...
- **SLOW / plot tier — in-loop ONLY (S28/S28a).** On cadence `eval.slow_every` (a multiple
  ...
  matplotlib render + `wandb.log` run on a rank-0 background thread (`BackgroundRenderer`)
