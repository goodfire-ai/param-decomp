...
def test_scale_invariance_per_component_exact_across_wide_range():
    ...
    base = _loss(u, AttentionHeads(4), 2.0)
    for scale in (1e-6, 1e-3, 137.0, 1e6):
        assert _loss(scale * u, AttentionHeads(4), 2.0) == pytest.approx(base, rel=1e-5)
...
def test_unit_blocks_are_contiguous():
    ...
    assert _loss(u, AttentionHeads(4), 4.0) == pytest.approx(1 / (1 + 4.0 / 4), rel=1e-6)
    # the same mass straddling units 0 and 1 -> two half-loaded units, higher count
    straddle = jnp.zeros((1, 12)).at[0, 2:5].set(1.0)
    assert _loss(straddle, AttentionHeads(4), 4.0) > _loss(u, AttentionHeads(4), 4.0)
...
def test_site_spec_rejects_bad_partitions():
    with pytest.raises(AssertionError):  # 10 % 4 != 0
        SiteSpec("s", 4, 10, 3, "s", nonlinearity_partition=AttentionHeads(4))
...
def _loss_metrics(
    ...
    return [
        ...
        SmoothL0ImportanceMinimalityLossConfig(coeff=1.0, gamma=schedule),
...
def test_component_nonlinearity_stats_endpoints_and_long_tail():
    ...
    assert float(one_hot.effective_unit_count_per_subcomponent[0]) == pytest.approx(1.0)

    uniform = component_nonlinearity_stats(jnp.ones((1, u_count)), Neurons())
    assert float(uniform.effective_unit_count_per_subcomponent[0]) == pytest.approx(float(u_count))

    zero = component_nonlinearity_stats(jnp.zeros((1, u_count)), Neurons())
    assert float(zero.soft_unit_count[0]) == 0.0
    assert float(zero.effective_unit_count_per_subcomponent[0]) == 0.0
    ...
    assert float(tail_stats.effective_unit_count_per_subcomponent[0]) == pytest.approx(
        expected, rel=1e-6
    )
...
def test_nonlinearity_log_aggregates_pool_components_within_unit_kind():
    stats = {
        "n0": ComponentNonlinearityStats(
            soft_unit_count=jnp.array([1.0, 3.0]),
            effective_unit_count_per_subcomponent=jnp.array([2.0, 4.0]),
        ),
        "n1": ComponentNonlinearityStats(
            soft_unit_count=jnp.array([5.0, 7.0]),
            effective_unit_count_per_subcomponent=jnp.array([6.0, 8.0]),
        ),
        "h0": ComponentNonlinearityStats(
            soft_unit_count=jnp.array([9.0, 11.0]),
            effective_unit_count_per_subcomponent=jnp.array([10.0, 12.0]),
        ),
    }
    ci_means = {
        "n0": np.array([1.0, 0.0]),
