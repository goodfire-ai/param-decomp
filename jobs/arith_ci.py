"""Causal importance of every p-530237f8 subcomponent at the '=' position of the 10000
...
Uses the pd-user run-dir copy (canonical goodfire tree unreadable by pd-user) via the
runs/p-530237f8 symlink; loader is this branch's open_jax_run (restore_forward_state
partial-restores components + ci_fn onto one GPU).
"""
...
RUN_DIR = Path("/mnt/home/pd-user/param-decomp-runs/runs/p-530237f8")
STEP = 400000
BATCH = 500
...
def main() -> None:
    ...
    for name, _ in sites:
        ...
        n_loose = int((m > 0.5).sum())
        n_strict = int(((eq_ci[name] >= 0.9).sum(axis=0) >= 10).sum())
        print(
            f"  {name}: {n_loose} comps with max CI > 0.5; "
            f"{n_strict} with CI >= 0.9 on >= 10 prompts (global max {m.max():.4g})",
            flush=True,
        )
