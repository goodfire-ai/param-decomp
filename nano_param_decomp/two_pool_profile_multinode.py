"""Multi-node wall-clock profile of stage 5 at ~400M target / ~3B CI fn scale.

16 GPUs across 2 nodes. Topology deliberately designed to MAXIMIZE cross-node comm
exposure so we get a real measurement of IB cost vs the NVLink numbers from
`two_pool_profile_400m.py`.

Pool A (12 ranks) and pool B (4 ranks) are interleaved across the two nodes so that:
  - Every in-block DDP all-reduce (6 block groups, DDP-2 each) crosses nodes.
  - Pool B DP-4 all-reduce crosses nodes.
  - Cross-pool sends from block leaders (all on node 0) go BOTH to node-0 B ranks (intra)
    AND to node-1 B ranks (inter) — direct intra-vs-inter comparison.

Rank assignment (SLURM allocates ranks 0..7 → node 0, 8..15 → node 1):
  node 0 GPUs: pool A ranks {0,1,2,3,4,5} + pool B ranks {6,7}
  node 1 GPUs: pool A ranks {8,9,10,11,12,13} + pool B ranks {14,15}

Block groups (all cross-node):
  (0,8), (1,9), (2,10), (3,11), (4,12), (5,13)

12 transformer blocks, 2 per block group → each pool-A rank owns 14 sites. M_local=14.

Run:
    sbatch nano_param_decomp/two_pool_profile_multinode.sbatch
"""
...
# Pool A interleaved across nodes; pool B split too.
# Node 0 ranks: 0-5 (pool A), 6-7 (pool B)
# Node 1 ranks: 8-13 (pool A), 14-15 (pool B)
POOL_A_RANKS = [0, 1, 2, 3, 4, 5, 8, 9, 10, 11, 12, 13]
POOL_B_RANKS = [6, 7, 14, 15]
WORLD_SIZE = 16
...
# --- Model dimensions (same as 400m profile) ---

VOCAB = 32000
D_MODEL = 1280
N_HEADS = 16
D_MLP = 5120
BATCH = 8  # divisible by both n_per_block_group (2) and n_pool_b (4)
SEQ_LEN = 64
C = 64
CI_HIDDEN = 22000
...
def main() -> None:
    ...
    assert world_size == WORLD_SIZE, f"need {WORLD_SIZE} ranks (got {world_size})"
    ...
    print(
        f"[rank{rank} node{node_id}] pool={layout.my_pool} "
