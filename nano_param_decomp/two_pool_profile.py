"""Wall-clock profile of stage 5 on a not-tiny model.

Instruments every per-step phase with `torch.cuda.synchronize()` + `time.perf_counter()`
to measure where wall-time actually goes. The goal is to validate the central
assumption of the 2-pool architecture — that pool A's layerwise compute is comparable
to pool B's PPGD compute, so neither pool starves the other.

Same topology as stage 5: 3 blocks × 2 ranks per block + 2 pool B ranks DP-2 = 8 GPUs.
Bigger model than the tiny stage-5 default so that compute is meaningful relative to
comm latency.

Optional: set `CUDA_LAUNCH_BLOCKING=1` in the environment for fully synchronous
kernel launches (makes phase boundaries strict but slightly slower).

Run:
    .venv/bin/python -m torch.distributed.run --standalone --nproc_per_node=8 \\
        -m nano_param_decomp.two_pool_profile
"""
...
# --- Model dimensions — bumped up from stage 5's tiny defaults ---
...
class StepTimer:
    ...
    Accumulates per-phase times across steps, skips a configurable warmup, and prints
    avg / min / max at end.
    """
    ...
    def report(self, warmup: int) -> None:
        ...
        total_avg = 0.0
        # Preserve insertion order so output reads chronologically.
        for name, vals in self.times.items():
            ...
            total_avg += avg
            print(
                f"  {name:32s} avg={avg:8.2f}ms  min={mn:8.2f}ms  max={mx:8.2f}ms  (n={len(vals)})",
                flush=True,
            )
        print(f"  {'TOTAL_PER_STEP':32s} avg={total_avg:8.2f}ms", flush=True)
...
def main() -> None:
    ...
    torch.manual_seed(0)
    target = Tiny6BlockTransformer(VOCAB, D_MODEL, N_BLOCKS, N_HEADS, D_MLP)
    ...
    if rank == 0:
        ...
        wrapper_params = sum(p.numel() for w in wrappers.values() for p in (w.V, w.U))
        ci_fn_params = sum(p.numel() for f in ci_fns.values() for p in f.parameters())
        print(
            f"[rank0] target_params={target_params:,} | "
            f"per-rank wrapper_params={wrapper_params:,} | "
            f"per-rank ci_fn_params={ci_fn_params:,}",
            flush=True,
        )
        print(f"[rank0] batch={BATCH} seq={SEQ_LEN} d_model={D_MODEL} d_mlp={D_MLP}", flush=True)
