"""Aggressive pool-A parallelism: 18 pool-A ranks (1 block each), 6 pool-B ranks DP-6.

24 GPUs across 3 nodes. Same 1B target / 10B CI fn model as
`two_pool_profile_stress.py`. But:

  * Pool A: 18 ranks, each owning ONE transformer block (7 sites). No in-block DDP.
    → in_block_allreduce becomes a no-op (single-rank group).
    → per-rank layerwise sites drops from 21 → 7.
    → per-rank CI fn drops from 1.65B → 553M.
  * Pool B: 6 ranks DP-6 (vs 4 ranks DP-4). Pool B per-rank compute drops.

Block groups round-robin across 3 nodes so each transformer block's owning rank lives
on a different node from its neighbours' owners; cross-pool sends span nodes maximally.

Rank assignment:
  node 0 (ranks 0-7):  pool A {0..5}, pool B {6,7}
  node 1 (ranks 8-15): pool A {8..13}, pool B {14,15}
  node 2 (ranks 16-23):pool A {16..21}, pool B {22,23}

Block → owning rank (round-robin across nodes):
  block i → node (i%3), pos (i//3) → rank = (i%3)*8 + (i//3)

Run:
    sbatch nano_param_decomp/two_pool_profile_stress_18way.sbatch
"""
...
# --- Same model dimensions as stress ---
...
def pool_a_step(
    ...
    with timer.phase("a/in_block_allreduce"):
        # With 1-rank block groups this is a no-op.
...
def main() -> None:
    ...
    block_owned_sites = [sites_for_block(i) for i in range(N_TRANSFORMER_BLOCKS)]
