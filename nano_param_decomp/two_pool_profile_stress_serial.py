"""Serial baseline of the 1B / 10B stress test.

Same hardware topology, same model, same comm primitives as
`two_pool_profile_stress.py`. The ONLY difference: pool A waits for pool B's grads
BEFORE starting home loss forwards (faith + imp + layerwise). This removes the
parallelism gained from overlapping pool A's home_fwd with pool B's PPGD compute.

Tells us how much wall-time the parallel 2-pool design actually saves at this scale.

Run:
    sbatch nano_param_decomp/two_pool_profile_stress_serial.sbatch
"""
...
# --- Topology / dimensions (identical to stress test) ---

POOL_A_RANKS = [0, 1, 2, 3, 4, 5, 8, 9, 10, 11, 12, 13]
POOL_B_RANKS = [6, 7, 14, 15]
WORLD_SIZE = 16
BLOCK_GROUPS = [[0, 8], [1, 9], [2, 10], [3, 11], [4, 12], [5, 13]]
...
def pool_a_step_serial(
    ...
    """SERIAL VARIANT: pool A waits for pool B's grads before computing home losses.

    Compare to pool_a_step in two_pool_profile_stress.py where home losses are computed
    immediately after send_ci_to_b, overlapping with pool B's PPGD work.
    """
    ...
    # *** SERIAL VARIANT: wait for B BEFORE computing home losses ***
    with timer.phase("a/recv_grads_from_b"):
        v_grads, u_grads, ci_grads = layout.recv_grads_from_pool_b(wrappers, ci_lower_owned)

    # NOW compute home losses (these used to overlap with pool B's work)
...
def pool_b_step(
    ...
    """Unchanged from parallel variant — pool B doesn't know about the serial sync."""
...
def main() -> None:
    ...
    if rank == 0:
        ...
        print(
            ...
            f"total_ci_fn={per_rank_ci_fn_params * len(BLOCK_GROUPS):,}",
            flush=True,
        )
