"""Stress-test profile: 1B target / 10B CI fn on 2 nodes × 8 H200.

Same topology as `two_pool_profile_multinode.py` (12A + 4B, all collectives cross-node).
Dimensions scaled up to validate analytic headroom:

  target ≈ 1.04B  (18 blocks, d=1792, d_mlp=7168, vocab=32000)
  ci_fn total ≈ 10B  (h=30000, 126 sites, ~79M per site)
  per pool-A rank: ~21 sites × 79M = 1.66B CI fn (~26GB AdamW state)

Each pool-A rank does 21 serial layerwise forwards on full target. Per-step wall-time
expected ~1.5-2.5s (vs 395ms at 350M scale).

Run:
    sbatch nano_param_decomp/two_pool_profile_stress.sbatch
"""
...
# --- Topology (same as multinode profile) ---
...
def main() -> None:
    ...
    for step in range(cfg.n_steps):
        ...
        if rank in (0, POOL_B_RANKS[0]):
            ...
            print(f"[rank{rank}] step={step} allocated={mem:.2f}GB peak={peak:.2f}GB", flush=True)
