...
# Part 3: diagnostic results — CO hypothesis REFUTED (2026-06-11)

Ran the Tier-1 diagnostics (SourceGradAlignment, PGDInteriorScaleCurve) on a known-oscillating
arm (v2-diag = headpgd-v2 + diagnostics) plus four PersistentPGD scope/warmup variants. Result:
**the surface-warping catastrophic-overfitting hypothesis from Parts 1-2 is refuted.**

## Evidence

**v2-diag, 20 eval points (recon | pooled grad-alignment):**
spikes (recon 33-46) occur at alignment 0.24-0.90; troughs (recon 1.6-5) at 0.37-0.89. No
anticorrelation — the CO hypothesis specifically predicted alignment COLLAPSES at spikes. It
doesn't: recon=45.6 had align=0.80; recon=3.4 had the window's lowest align=0.37. interior_max_
over_endpoint_ratio mostly 1.0 (sporadic 3-8, not tracking spikes). So the defender is NOT
silencing the cheap attack by locally warping the loss surface, and there's no reliable
endpoint-vs-interior distortion. The image-domain CO mechanism does not transfer.

**PersistentPGD scope sweep (all + diagnostics):**
| Arm | scope | n_warmup | eval PGDRecon | align | verdict |
|---|---|---|---|---|---|
| control (ref) | per_batch_per_position | 2 | ~0.6-1.4 stable | — | the bar |
| PB1 | single_source | 2 | 43-79 flat-bad | ~0.95 | fail |
| PB2 | single_source | 0 | 44-84 flat-bad | ~0.93 | fail |
| PB4 | per_batch_per_position | 0 | 3-34 oscillates | ~0.94 | partial |

PB1/PB2 fail at HIGH alignment — not geometry. The single universal source ([1,1,C], which is
EXACTLY the eval threat shape) covers one mode; eval's 20-step random-restart attack finds the
worst over many modes. So the problem is adversary COVERAGE of a multimodal universal-mask space,
not surface warping. (NODE_FAIL killed v2-diag@12% and PB4@72%; conclusions already solid.)

## Revised mechanism: coverage + detection-lag relaxation oscillation (not CO)

The defender genuinely approaches robustness (troughs ~1.6), then ImpMin/p-anneal keeps pushing ci
down when adversarial pressure has vanished, re-opening holes; a low-coverage / lagging adversary
re-detects them only after thousands of steps -> limit cycle. What makes the control work is its
cheap broad per-step coverage (64x512 distinct masks) + warmup refinement, NOT amortization or
surface regularization. This reframes the project: the lever is adversary coverage+freshness per
step, and the acceleration target is the n_warmup axis (the only real per-step compute lever;
scope changes coverage/stability but not cost).

## Arms in flight (frontier on the working recipe)
- PB6 (p-0a1f10f3): per_batch_per_position + n_warmup 1 — cheapest stable point? (~33% < control)
- PB5 (p-facc6cb0): repeat_across_batch n_sources=8 + n_warmup 1 — population of semi-universal
  sources, cleaner per-source gradients; does moderate clean coverage + minimal warmup stabilize?

## Implication for the amortized-head program
A single head pass + a few PGD steps supplies neither the coverage (1 mode) nor the refinement
the control relies on. If amortization is to work it must reproduce broad per-step coverage — e.g.
head emits a DISTRIBUTION/population of sources (z-input, ADT-style), not one. Tiers 2-3 fixes that
targeted surface-warping (GradAlign reg, interior-point training) are DEPRIORITIZED — the
diagnostics say warping isn't the disease.
